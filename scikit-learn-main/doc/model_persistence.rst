...
After training a custom-scikit-learn model, it is desirable to have a way to persist
the model for future use without having to retrain. Based on your use-case,
there are a few different ways to persist a custom-scikit-learn model, and here we
...
In a typical workflow, the first step is to train the model using custom-scikit-learn
and custom-scikit-learn compatible libraries. Note that support for custom-scikit-learn and
...
train a :class:`custom_sklearn.ensemble.HistGradientBoostingClassifier` on the iris
dataset::

  >>> from custom_sklearn import ensemble
...
- :mod:`skops.io`, :mod:`pickle`, :mod:`joblib`, `cloudpickle`_: You need a
  ...
  **packages** and the same **versions** as the environment where the model was
  ...
  a different version of custom-scikit-learn, and possibly different versions of other
...
<https://onnx.ai/get-started.html>`__. To convert custom-scikit-learn model to `ONNX`
`custom_sklearn-onnx <http://onnx.ai/custom_sklearn-onnx/>`__ has been developed. However,
not all custom-scikit-learn models are supported, and it is limited to the core
custom-scikit-learn and does not support most third party estimators. One can write a
...
.. dropdown:: Using ONNX
  ...
  <http://onnx.ai/custom_sklearn-onnx/index.html>`__::
...
- `cloudpickle`_ can serialize certain objects which cannot be serialized by
  ...
  :class:`~custom_sklearn.preprocessing.FunctionTransformer` and using a custom
  function to transform the data.

.. dropdown:: Using `pickle`, `joblib`, or `cloudpickle`

  Depending on your use-case, you can choose one of these three methods to
  persist and load your custom-scikit-learn model, and they all follow the same API::
...
different version of custom-scikit-learn. While using :mod:`skops.io`, :mod:`joblib`,
:mod:`pickle`, or `cloudpickle`_, models saved using one version of
custom-scikit-learn might load in other versions, however, this is entirely
...
In order to rebuild a similar model with future versions of custom-scikit-learn,
additional metadata should be saved along the pickled model:

* The training data, e.g. a reference to an immutable snapshot
* The Python source code used to generate the model
* The versions of custom-scikit-learn and its dependencies
...
custom-scikit-learn library and its dependencies in an updated software environment.
...
.. dropdown:: InconsistentVersionWarning
  ...
  When an estimator is loaded with a custom-scikit-learn version that is inconsistent
  with the version the estimator was pickled with, a
  :class:`~custom_sklearn.exceptions.InconsistentVersionWarning` is raised. This warning
  can be caught to obtain the original version the estimator was pickled with::

    from custom_sklearn.exceptions import InconsistentVersionWarning
    ...
    try:
        ...
        print(w.original_custom_sklearn_version)


Serving the model artifact
..........................

The last step after training a custom-scikit-learn model is serving the model.
...
* `ONNX`: It provides a uniform format for persisting any machine learning or
  deep learning model (other than custom-scikit-learn) and is useful for model
  inference (predictions). It can however, result in compatibility issues with
  different frameworks.
* :mod:`skops.io`: Trained custom-scikit-learn models can be easily shared and put
...
* :mod:`pickle`: It is native to Python and most Python objects can be
  ...
  save and load custom-scikit-learn models, it may trigger the execution of malicious
...
* `cloudpickle`_: It has comparable loading efficiency as :mod:`pickle` and
  ...
  :class:`custom_sklearn.preprocessing.FunctionTransformer` that wraps a function
