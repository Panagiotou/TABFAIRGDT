...
Some custom-scikit-learn estimators and utilities parallelize costly operations
...
The thread-level parallelism managed by OpenMP in custom-scikit-learn's own Cython code
or by BLAS & LAPACK libraries used by NumPy and SciPy operations used in custom-scikit-learn
...
.. note::
    ...
    <https://github.com/custom-scikit-learn/custom-scikit-learn/issues/14228>`_!
...
custom-scikit-learn generally relies on the ``loky`` backend, which is joblib's
...
GIL), custom-scikit-learn will indicate to ``joblib`` that a multi-threading
backend is preferable.

As a user, you may control the backend that joblib will use (regardless of
what custom-scikit-learn recommends) by using a context manager::

    from joblib import parallel_backend

    with parallel_backend('threading', n_jobs=2):
        # Your custom-scikit-learn code here
...
custom-scikit-learn relies heavily on NumPy and SciPy, which internally call
...
a :class:`~custom_sklearn.model_selection.GridSearchCV` (parallelized with joblib)
with ``n_jobs=8`` over a
:class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` (parallelized with
OpenMP). Each instance of
:class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` will spawn 8 threads
...
:class:`~custom_sklearn.model_selection.GridSearchCV` is ``loky``, each process will
...
- All custom-scikit-learn estimators that explicitly rely on OpenMP in their Cython code
...
:func:`custom_sklearn.set_config` and :func:`custom_sklearn.config_context` can be used to change
...
These environment variables should be set before importing custom-scikit-learn.

`custom_sklearn_ASSUME_FINITE`
~~~~~~~~~~~~~~~~~~~~~~~

Sets the default value for the `assume_finite` argument of
:func:`custom_sklearn.set_config`.

`custom_sklearn_WORKING_MEMORY`
~~~~~~~~~~~~~~~~~~~~~~~~
...
Note that custom-scikit-learn tests are expected to run deterministically with
...
`custom_sklearn_TESTS_GLOBAL_RANDOM_SEED`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
If the `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED` environment variable is set to
...
Valid values for `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED`:

- `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="42"`: run tests with a fixed seed of 42
- `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="40-42"`: run the tests with all seeds
  between 40 and 42 included
- `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="any"`: run the tests with an arbitrary
  seed selected between 0 and 99 included
- `custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="all"`: run the tests with all seeds
...
deterministic manner. This ensures that, by default, the custom-scikit-learn test
...
unrelated to the changes of their own PR. Only the custom-scikit-learn maintainers who
...
.. prompt:: bash $
    ...
    custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="all" pytest -v -k test_your_test_name

`custom_sklearn_SKIP_NETWORK_TESTS`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`custom_sklearn_RUN_FLOAT32_TESTS`
~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`custom_sklearn_ENABLE_DEBUG_CYTHON_DIRECTIVES`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`custom_sklearn_BUILD_ENABLE_DEBUG_SYMBOLS`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`custom_sklearn_PAIRWISE_DIST_CHUNK_SIZE`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
`custom_sklearn_WARNINGS_AS_ERRORS`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This environment variable is used to turn warnings into errors in tests and
documentation build.

Some CI (Continuous Integration) builds set `custom_sklearn_WARNINGS_AS_ERRORS=1`, for
...
you can set `custom_sklearn_WARNINGS_AS_ERRORS=1`.

By default, warnings are not turned into errors. This is the case if
`custom_sklearn_WARNINGS_AS_ERRORS` is unset, or `custom_sklearn_WARNINGS_AS_ERRORS=0`.
...
`_get_warnings_filters_info_list` function in `custom_sklearn/utils/_testing.py`.

Note that for documentation build, `custom_sklearn_WARNING_AS_ERRORS=1` is checking
