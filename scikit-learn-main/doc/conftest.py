...
from custom_sklearn.datasets._base import _pkl_filepath
from custom_sklearn.datasets._twenty_newsgroups import CACHE_NAME
from custom_sklearn.utils._testing import SkipTest, check_skip_network
from custom_sklearn.utils.fixes import _IS_PYPY, np_base_version, parse_version
...
def setup_loading_other_datasets():
    ...
    # checks custom_sklearn_SKIP_NETWORK_TESTS to see if test should run
    run_network_tests = environ.get("custom_sklearn_SKIP_NETWORK_TESTS", "1") == "0"
    if not run_network_tests:
        raise SkipTest(
            "Skipping loading_other_datasets.rst, tests can be "
            "enabled by setting custom_sklearn_SKIP_NETWORK_TESTS=0"
        )
...
def pytest_collection_modifyitems(config, items):
    ...
    if np_base_version >= parse_version("2"):
        ...
        # https://github.com/custom-scikit-learn/custom-scikit-learn/issues/27339
