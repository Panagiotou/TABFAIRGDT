...
==================================================
Installing the development version of custom-scikit-learn
==================================================

This section introduces how to install the **main branch** of custom-scikit-learn.
...
The continuous integration servers of the custom-scikit-learn project build, test
...
You can install the nightly build of custom-scikit-learn using the `scientific-python-nightly-wheels`
index from the PyPI registry of `anaconda.org`:

.. prompt:: bash $

  pip install --pre --extra-index https://pypi.anaconda.org/scientific-python-nightly-wheels/simple custom-scikit-learn

Note that first uninstalling custom-scikit-learn might be required to be able to
install nightly builds of custom-scikit-learn.
...
#. Use `Git <https://git-scm.com/>`_ to check out the latest source from the
   `custom-scikit-learn repository <https://github.com/custom-scikit-learn/custom-scikit-learn>`_ on
   Github.:

   .. prompt:: bash $

     git clone git://github.com/custom-scikit-learn/custom-scikit-learn.git  # add --depth 1 if your connection is slow
...
#. Install a recent version of Python (3.9 is recommended at the time of writing)
   ...
   `conda environment`_ with all the build dependencies of custom-scikit-learn
   (namely NumPy_, SciPy_, Cython_, meson-python_ and Ninja_):

   .. prompt:: bash $

     conda create -n custom_sklearn-env -c conda-forge python=3.9 numpy scipy cython meson-python ninja
...
#. **Alternative to conda:** You can use alternative installations of Python
   ...
   .. prompt:: bash $
     ...
     source custom_sklearn-env/bin/activate
...
#. Check that the installed custom-scikit-learn has a version number ending with
   `.dev0`:
...
.. note::
    ...
    to avoid surprises when you import `custom_sklearn`. `meson-python` implements
    editable installs by rebuilding `custom_sklearn` when executing `import custom_sklearn`.
    ...
    install` command once, `custom_sklearn` will automatically be rebuilt when
    importing `custom_sklearn`.
...
custom-scikit-learn requires the following dependencies both at build time and at
runtime:
...
Building custom-scikit-learn also requires:

..
    # The following places need to be in sync with regard to Cython version:
    # - .circleci config file
    # - custom_sklearn/_build_utils/__init__.py
...
.. note::
   ...
   parallelism. Setting the ``custom_sklearn_FAIL_NO_OPENMP`` environment variable
   (before cythonization) will force the build to fail if OpenMP is not
   supported.

Since version 0.21, custom-scikit-learn automatically detects and uses the linear
algebra library used by SciPy **at runtime**. custom-scikit-learn has therefore no
...
to build custom-scikit-learn Cython extensions for each supported platform.
...
Finally, build custom-scikit-learn with this command prompt:
...
custom-scikit-learn from source:

.. prompt:: bash $

    conda create -n custom_sklearn-dev -c conda-forge python numpy scipy cython \
...
.. prompt:: bash $
    ...
    conda activate custom_sklearn-dev
    make clean
    pip install --editable . \
        --verbose --no-build-isolation \
        --config-settings editable-verbose=true
...
They point to files and folders from your ``custom_sklearn-dev`` conda environment
(in particular in the bin/, include/ and lib/ subfolders). For instance
``-L/path/to/conda/envs/custom_sklearn-dev/lib`` should appear in ``LDFLAGS``.
...
Finally, build custom-scikit-learn in verbose mode (to check for the presence of the
...
Installing custom-scikit-learn from source without using conda requires you to have
installed the custom-scikit-learn Python development headers and a working C/C++
...
The following command will build custom-scikit-learn using your default C/C++ compiler.
...
If you want to build custom-scikit-learn with another compiler handled by ``setuptools``,
...
In addition, since custom-scikit-learn uses OpenMP, you need to include the appropriate OpenMP
