...
Whether you are proposing an estimator for inclusion in custom-scikit-learn,
developing a separate package compatible with custom-scikit-learn, or
implementing custom components for your own projects, this chapter
details how to develop objects that safely interact with custom-scikit-learn
...
APIs of custom-scikit-learn objects
============================
...
Elements of the custom-scikit-learn API are described more definitively in the
:ref:`glossary`.

Different objects
-----------------

The main objects in custom-scikit-learn are (one class can implement
multiple interfaces):
...
All estimators in the main custom-scikit-learn codebase should inherit from
``custom_sklearn.base.BaseEstimator``.
...
correspond to an attribute on the instance**. custom-scikit-learn relies on this to
...
See
`SLEP010
<https://custom-scikit-learn-enhancement-proposals.readthedocs.io/en/latest/slep010/proposal.html>`_
for details.
...
==========================
If you want to implement a new estimator that is custom-scikit-learn-compatible,
whether it is just for you or for contributing it to custom-scikit-learn, there are
several internals of custom-scikit-learn that you should be aware of in addition to
the custom-scikit-learn API outlined above. You can check whether your estimator
adheres to the custom-scikit-learn interface and standards by running
:func:`~custom_sklearn.utils.estimator_checks.check_estimator` on an instance. The
:func:`~custom_sklearn.utils.estimator_checks.parametrize_with_checks` pytest
decorator can also be used (see its docstring for details and possible
interactions with `pytest`)::

  >>> from custom_sklearn.utils.estimator_checks import check_estimator
  >>> from custom_sklearn.svm import LinearSVC
  >>> check_estimator(LinearSVC())  # passes

The main motivation to make a class compatible to the custom-scikit-learn estimator
...
.. topic:: Project template:
    ...
    We provide a `project template <https://github.com/custom-scikit-learn-contrib/project-template/>`_
    which helps in the creation of Python packages containing custom-scikit-learn compatible estimators.
    It provides:

    * an initial git repository with Python package directory structure
    * a template of a custom-scikit-learn estimator
...
.. topic:: ``BaseEstimator`` and mixins:
    ...
    even import any custom-scikit-learn classes.

    However, if a dependency on custom-scikit-learn is acceptable in your code,
    you can prevent a lot of boilerplate code
    by deriving a class from ``BaseEstimator``
    and optionally the mixin classes in ``custom_sklearn.base``.
    For example, below is a custom classifier, with more examples included
    in the custom-scikit-learn-contrib
    `project template <https://github.com/custom-scikit-learn-contrib/project-template/blob/master/skltemplate/_template.py>`__.
    ...
    MRO.
      ...
      >>> import numpy as np
      >>> from custom_sklearn.base import BaseEstimator, ClassifierMixin
      >>> from custom_sklearn.utils.validation import check_X_y, check_array, check_is_fitted
      >>> from custom_sklearn.utils.multiclass import unique_labels
      >>> from custom_sklearn.metrics import euclidean_distances
...
-------------------------
All custom-scikit-learn estimators have ``get_params`` and ``set_params`` functions.
...
estimator::
    ...
    >>> from custom_sklearn.base import BaseEstimator
...
:class:`~custom_sklearn.pipeline.Pipeline` object), in which case the key should
...
``__repr__`` method, is to inherit from ``custom_sklearn.base.BaseEstimator``. If you
do not want to make your code dependent on custom-scikit-learn, the easiest way to
...
Cloning
-------
For use with the :mod:`~custom_sklearn.model_selection` module,
...
`__custom_sklearn_clone__` method. `__custom_sklearn_clone__` must return an instance of the
estimator. `__custom_sklearn_clone__` is useful when an estimator needs to hold on to
...
frozen meta-estimator for transformers can be defined as follows::
    ...
    class FrozenTransformer(BaseEstimator):
        ...
        def __custom_sklearn_clone__(self):
            return self
...
independent term is stored in ``intercept_``.  ``custom_sklearn.linear_model._base``
contains a few base classes and mixins that implement common linear model
patterns.

The :mod:`~custom_sklearn.utils.multiclass` module contains useful functions
...
custom-scikit-learn introduced estimator tags in version 0.21. These are annotations
...
:func:`~custom_sklearn.utils.estimator_checks.check_estimator` function and the
:func:`~custom_sklearn.utils.estimator_checks.parametrize_with_checks` decorator.
...
pairwise (default=False)
    ...
    `custom_sklearn.utils.metaestimators._safe_split` to slice rows and
    columns.
...
poor_score (default=False)
    ...
    are based on current estimators in custom_sklearn and might be replaced by
...
requires_y (default=False)
    ...
    `~custom_sklearn.base.RegressorMixin` and `~custom_sklearn.base.ClassifierMixin`.
...
_xfail_checks (default=False)
    ...
    :func:`~custom_sklearn.utils.estimator_checks.parametrize_with_checks`. These
    checks will be simply ignored and not run by
    :func:`~custom_sklearn.utils.estimator_checks.check_estimator`, but a
...
With
`SLEP018 <https://custom-scikit-learn-enhancement-proposals.readthedocs.io/en/latest/slep018/proposal.html>`__,
custom-scikit-learn introduces the `set_output` API for configuring transformers to
...
By default :func:`~custom_sklearn.utils.validation.check_is_fitted` checks if there
are any attributes in the instance with a trailing underscore, e.g. `coef_`.
An estimator can change the behavior by implementing a `__custom_sklearn_is_fitted__`
method taking no input and returning a boolean. If this method exists,
:func:`~custom_sklearn.utils.validation.check_is_fitted` simply returns its output.

See :ref:`sphx_glr_auto_examples_developing_estimators_custom_sklearn_is_fitted.py`
for an example on how to use the API.
...
Estimators inheriting from :class:`~custom_sklearn.base.BaseEstimator` display
...
:func:`~custom_sklearn.utils.estimator_html_repr` on an estimator instance.
...
contain a link to the documentation. For custom-scikit-learn estimators this is set to
`"custom_sklearn"`.
...
inclusion in custom-scikit-learn, and which may be appropriate to adopt in external
...
custom-scikit-learn project tries to closely follow the official Python guidelines
...
* Use relative imports for references inside custom-scikit-learn.

* Unit tests are an exception to the previous rule;
  they should use absolute imports, exactly as client code would.
  A corollary is that, if ``custom_sklearn.foo`` exports a class or function
  that is implemented in ``custom_sklearn.foo.bar.baz``,
  the test should import it from ``custom_sklearn.foo``.
...
* **Please don't use** ``import *`` **in any case**. It is considered harmful
  ...
  find bugs in custom-scikit-learn.
...
.. currentmodule:: custom_sklearn.utils

The module :mod:`custom_sklearn.utils` contains various functions for doing input
...
passed to a custom-scikit-learn API function. The exact parameters to use depends
...
See :func:`custom_sklearn.utils.check_random_state` in :ref:`developers-utils`.

Here's a simple example of code using some of the above guidelines::

    from custom_sklearn.utils import check_array, check_random_state
...
The following example should make this clear::
    ...
    class GaussianNoise(BaseEstimator, TransformerMixin):
        ...
        It also does not adhere to all custom-scikit-learn conventions,
...
do use `custom_sklearn.utils._testing.assert_allclose`.
...
`custom_sklearn.utils._testing.assert_allclose`.
