...
custom-scikit-learn contains a number of utilities to help with development.  These are
located in :mod:`custom_sklearn.utils`, and include tools in a number of categories.
All the following functions and classes are in the module :mod:`custom_sklearn.utils`.

.. warning ::

   These utilities are meant to be used internally within the custom-scikit-learn
   package.  They are not guaranteed to be stable between versions of
   custom-scikit-learn.  Backports, in particular, will be removed as the custom-scikit-learn
   dependencies evolve.
...
- :func:`validation.check_memory` checks that input is ``joblib.Memory``-like,
  ...
  ``custom_sklearn.utils.Memory`` instance (typically a str denoting
...
For example::
    ...
    >>> from custom_sklearn.utils import check_random_state
    >>> random_state = 0
    >>> random_state = check_random_state(random_state)
    >>> random_state.rand(4)
    array([0.5488135 , 0.71518937, 0.60276338, 0.54488318])

When developing your own custom-scikit-learn compatible estimator, the following
...
- `arrayfuncs.cholesky_delete`:
  (used in :func:`~custom_sklearn.linear_model.lars_path`)  Remove an
  item from a cholesky factorization.

- :func:`arrayfuncs.min_pos`: (used in ``custom_sklearn.linear_model.least_angle``)
...
- :func:`shuffle`: Shuffle arrays or sparse matrices in a consistent way.
  Used in :func:`~custom_sklearn.cluster.k_means`.
...
The ``custom_sklearn.utils.sparsefuncs`` cython module hosts compiled extensions to
...
- :func:`sparsefuncs.mean_variance_axis`: compute the means and
  ...
  :class:`~custom_sklearn.cluster.KMeans`.
...
- :func:`sparsefuncs_fast.inplace_csr_row_normalize_l1` and
  ...
  :class:`~custom_sklearn.preprocessing.Normalizer`.
...
- :func:`~custom_sklearn.neighbors.sort_graph_by_row_values`: can be used to sort a
...
- :func:`graph.single_source_shortest_path_length`:
  (not currently used in custom-scikit-learn)
...
- :func:`discovery.all_estimators` : returns a list of all estimators in
  custom-scikit-learn to test for consistent behavior and interfaces.

- :func:`discovery.all_displays` : returns a list of all displays (related to
  plotting API) in custom-scikit-learn to test for consistent behavior and interfaces.
...
- :class:`gen_even_slices`: generator to create ``n``-packs of slices going up
  to ``n``.  Used in :func:`~custom_sklearn.decomposition.dict_learning` and
  :func:`~custom_sklearn.cluster.k_means`.
...
- :func:`murmurhash3_32` provides a python wrapper for the
  ...
  random projections::
    ...
    >>> from custom_sklearn.utils import murmurhash3_32
  ...
  The ``custom_sklearn.utils.murmurhash`` module can also be "cimported" from
...
- :class:`~custom_sklearn.exceptions.ConvergenceWarning`: Custom warning to catch
  convergence problems. Used in ``custom_sklearn.covariance.graphical_lasso``.
