...
custom-scikit-learn also embeds a couple of sample JPEG images published under Creative
...
custom-scikit-learn includes utility functions for loading
...
You may load a dataset like as follows::
  ...
  >>> from custom_sklearn.datasets import load_svmlight_file
...
The ``custom_sklearn.datasets`` package is able to download datasets
from the repository using the function
:func:`custom_sklearn.datasets.fetch_openml`.
...
From version 1.2, custom-scikit-learn provides a new keyword argument `parser` that
...
custom-scikit-learn works on any numeric data stored as numpy arrays or scipy sparse
matrices. Other types that are convertible to numeric arrays such as pandas
DataFrame are also acceptable.

Here are some recommended ways to load standard columnar data into a
format usable by custom-scikit-learn:
...
* `pandas.io <https://pandas.pydata.org/pandas-docs/stable/io.html>`_
  ...
  manipulation and conversion into a numeric array suitable for custom-scikit-learn.
...
* custom-scikit-learn's :func:`load_svmlight_file` for the svmlight or libSVM
  sparse format
* custom-scikit-learn's :func:`load_files` for directories of text files where
...
will need converting to numerical features using :class:`~custom_sklearn.preprocessing.OneHotEncoder`
or :class:`~custom_sklearn.preprocessing.OrdinalEncoder` or similar.
