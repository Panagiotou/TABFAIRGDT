...
Gaussian Mixtures (:class:`~custom_sklearn.mixture.GaussianMixture`), and
neighbor-based approaches such as the kernel density estimate
(:class:`~custom_sklearn.neighbors.KernelDensity`).
...
=========================
Kernel density estimation in custom-scikit-learn is implemented in the
:class:`~custom_sklearn.neighbors.KernelDensity` estimator, which uses the
...
distribution.  The custom-scikit-learn kernel density estimator can be used as
follows:

   >>> from custom_sklearn.neighbors import KernelDensity
   >>> import numpy as np
...
:class:`~custom_sklearn.neighbors.KernelDensity` implements several common kernel
...
metrics (see :class:`~custom_sklearn.metrics.DistanceMetric` for a list of
