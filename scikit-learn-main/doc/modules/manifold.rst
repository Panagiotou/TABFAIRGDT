...
.. currentmodule:: custom_sklearn.manifold
...
The manifold learning implementations available in custom-scikit-learn are
summarized below
...
.. dropdown:: Complexity
  ...
  1. **Nearest neighbor search.**  Isomap uses
     :class:`~custom_sklearn.neighbors.BallTree` for efficient neighbor search.
...
scaling with data size, ``custom_sklearn`` implements some algorithmic
...
custom-scikit-learn implements Laplacian Eigenmaps, which finds a low dimensional
...
custom-scikit-learn, the class :class:`MDS` implements both. In Metric MDS, the input
...
.. dropdown:: Barnes-Hut t-SNE
  ...
  * Barnes-Hut only works with dense input data. Sparse data matrices can only be
    ...
    projection for instance using :class:`~custom_sklearn.decomposition.PCA`
