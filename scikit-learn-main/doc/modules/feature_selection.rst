.. currentmodule:: custom_sklearn.feature_selection
...
The classes in the :mod:`custom_sklearn.feature_selection` module can be used
...
so we can select using the threshold ``.8 * (1 - .8)``::
  ...
  >>> from custom_sklearn.feature_selection import VarianceThreshold
...
to an estimator. custom-scikit-learn exposes feature selection routines
...
best features for a dataset as follows:
  ...
  >>> from custom_sklearn.feature_selection import f_classif
...
.. dropdown:: L1-recovery and compressive sensing
  ...
  (:class:`~custom_sklearn.linear_model.LassoCV` or
  :class:`~custom_sklearn.linear_model.LassoLarsCV`), though this may lead to
  under-penalized models: including a small number of non-relevant variables
  is not detrimental to prediction score. BIC
  (:class:`~custom_sklearn.linear_model.LassoLarsIC`) tends, on the opposite, to set
...
Tree-based estimators (see the :mod:`custom_sklearn.tree` module and forest
of trees in the :mod:`custom_sklearn.ensemble` module) can be used to compute
...
:class:`~custom_sklearn.feature_selection.SequentialFeatureSelector` transformer.
...
.. dropdown:: Details on Sequential Feature Selection
  ...
  SFS differs from :class:`~custom_sklearn.feature_selection.RFE` and
  :class:`~custom_sklearn.feature_selection.SelectFromModel` in that it does not
  ...
  :class:`~custom_sklearn.feature_selection.RFE` would require only a single fit, and
  :class:`~custom_sklearn.feature_selection.SelectFromModel` always just does a single
...
the actual learning. The recommended way to do this in custom-scikit-learn is
