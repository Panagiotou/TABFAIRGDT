...
.. currentmodule:: custom_sklearn.kernel_ridge
...
vector regression (:class:`~custom_sklearn.svm.SVR`). However, different loss
...
regularization. In contrast to :class:`~custom_sklearn.svm.SVR`, fitting
...
thus slower than :class:`~custom_sklearn.svm.SVR`, which learns a sparse model for
:math:`\epsilon > 0`, at prediction-time.

The following figure compares :class:`KernelRidge` and
:class:`~custom_sklearn.svm.SVR` on an artificial dataset, which consists of a
sinusoidal target function and strong noise added to every fifth datapoint.
The learned model of :class:`KernelRidge` and :class:`~custom_sklearn.svm.SVR` is
...
faster than fitting :class:`~custom_sklearn.svm.SVR` (both with grid-search).
However, prediction of 100000 target values is more than three times faster
with :class:`~custom_sklearn.svm.SVR` since it has learned a sparse model using only
...
:class:`KernelRidge` and :class:`~custom_sklearn.svm.SVR` for different sizes of the
training set. Fitting :class:`KernelRidge` is faster than
:class:`~custom_sklearn.svm.SVR` for medium-sized training sets (less than 1000
samples); however, for larger training sets :class:`~custom_sklearn.svm.SVR` scales
better. With regard to prediction time, :class:`~custom_sklearn.svm.SVR` is faster
...
the :class:`~custom_sklearn.svm.SVR`; :math:`\epsilon = 0` would correspond to a
dense model.
