...
.. currentmodule:: custom_sklearn.impute

For various reasons, many real world datasets contain missing values, often
encoded as blanks, NaNs or other placeholders. Such datasets however are
incompatible with custom-scikit-learn estimators which assume that all values in an
...
:func:`~custom_sklearn.metrics.pairwise.nan_euclidean_distances`, is used to find the
...
By default, the custom-scikit-learn imputers will drop fully empty features, i.e.
...
use ``-1`` as missing values::
  ...
  >>> from custom_sklearn.impute import MissingIndicator
  >>> X = np.array([[-1, -1, 1, 3],
  ...               [4, -1, 0, -1],
  ...               [8, -1, 1, 0]])
...
:class:`~custom_sklearn.pipeline.Pipeline`, be sure to use the
:class:`~custom_sklearn.pipeline.FeatureUnion` or
:class:`~custom_sklearn.compose.ColumnTransformer` to add the indicator features to
the regular features. First we obtain the `iris` dataset, and add some missing
values to it.

  >>> from custom_sklearn.datasets import load_iris
  >>> from custom_sklearn.impute import SimpleImputer, MissingIndicator
  >>> from custom_sklearn.model_selection import train_test_split
  >>> from custom_sklearn.pipeline import FeatureUnion, make_pipeline
...
Now we create a :class:`~custom_sklearn.pipeline.FeatureUnion`. All features will be
...
wrap this in a :class:`~custom_sklearn.pipeline.Pipeline` with a classifier (e.g., a
:class:`~custom_sklearn.tree.DecisionTreeClassifier`) to be able to make predictions.
