...
the custom-scikit-learn API, potentially using a different optimization technique.
For example, using `SGDClassifier(loss='log_loss')` results in logistic regression,
i.e. a model equivalent to :class:`~custom_sklearn.linear_model.LogisticRegression`
which is fitted via SGD instead of being fitted by one of the other solvers
in :class:`~custom_sklearn.linear_model.LogisticRegression`. Similarly,
`SGDRegressor(loss='squared_error', penalty='l2')` and
:class:`~custom_sklearn.linear_model.Ridge` solve the same optimization problem, via
different means.
...
The class :class:`custom_sklearn.linear_model.SGDOneClassSVM` implements an online
linear version of the One-Class SVM using a stochastic gradient descent.
Combined with kernel approximation techniques,
:class:`custom_sklearn.linear_model.SGDOneClassSVM` can be used to approximate the
solution of a kernelized One-Class SVM, implemented in
:class:`custom_sklearn.svm.OneClassSVM`, with a linear complexity in the number of
samples. Note that the complexity of a kernelized One-Class SVM is at best
quadratic in the number of samples.
:class:`custom_sklearn.linear_model.SGDOneClassSVM` is thus well suited for datasets
...
* Stochastic Gradient Descent is sensitive to feature scaling, so it
  ...
  done using :class:`~custom_sklearn.preprocessing.StandardScaler`::
...
* Finding a reasonable regularization term :math:`\alpha` is
  ...
  :class:`~custom_sklearn.model_selection.RandomizedSearchCV`, usually in the
