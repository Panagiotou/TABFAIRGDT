...
unlabeled data can be performed with the module :mod:`custom_sklearn.cluster`.
...
.. currentmodule:: custom_sklearn.cluster
...
.. topic:: Input data
    ...
    These can be obtained from the classes in the :mod:`custom_sklearn.feature_extraction`
    ...
    in the :mod:`custom_sklearn.metrics.pairwise` module.
...
.. figure:: ../auto_examples/cluster/images/sphx_glr_plot_cluster_comparison_001.png
   ...
   A comparison of the clustering algorithms in custom-scikit-learn
...
k-means++ initialization scheme, which has been implemented in custom-scikit-learn
...
clustering algorithms, see :func:`custom_sklearn.cluster.kmeans_plusplus` for details
and example usage.
...
graph, and SpectralClustering is initialized with `affinity='precomputed'`::
    ...
    >>> from custom_sklearn.cluster import SpectralClustering
...
using :func:`custom_sklearn.neighbors.kneighbors_graph` to restrict
merging to nearest neighbors as in :ref:`this example
<sphx_glr_auto_examples_cluster_plot_agglomerative_clustering.py>`, or
using :func:`custom_sklearn.feature_extraction.image.grid_to_graph` to
...
.. warning:: **Connectivity constraints with single, average and complete linkage**
    ...
    :func:`custom_sklearn.neighbors.kneighbors_graph`. In the limit of a small
...
.. dropdown:: Implementation
  ...
  was done in custom-scikit-learn versions before 0.14). The possibility to use custom
...
.. dropdown:: Memory consumption for large sample sizes
  ...
  - A sparse radius neighborhood graph (where missing entries are presumed to be
    ...
    :meth:`custom_sklearn.neighbors.NearestNeighbors.radius_neighbors_graph`.
...
.. note::
  ...
  `custom-scikit-learn-contrib/hdbscan <https://github.com/custom-scikit-learn-contrib/hdbscan>`_ based on [LJ2017]_.
...
The Fowlkes-Mallows index (:func:`custom_sklearn.metrics.fowlkes_mallows_score`) can be
...
Silhouette Coefficient for each sample.
  ...
  >>> from custom_sklearn import metrics
  >>> from custom_sklearn.metrics import pairwise_distances
...
(:func:`custom_sklearn.metrics.calinski_harabasz_score`) - also known as the Variance
...
(:func:`custom_sklearn.metrics.davies_bouldin_score`) can be used to evaluate the
...
cluster analysis as follows:
  ...
  >>> X = iris.data
  >>> from custom_sklearn.cluster import KMeans
  >>> from custom_sklearn.metrics import davies_bouldin_score
...
Contingency matrix (:func:`custom_sklearn.metrics.cluster.contingency_matrix`)
...
Here is an example::
   ...
   >>> from custom_sklearn.metrics.cluster import contingency_matrix
   >>> x = ["a", "a", "a", "b", "b", "b"]
   >>> y = [0, 0, 1, 1, 2, 2]
...
(:func:`custom_sklearn.metrics.cluster.pair_confusion_matrix`) is a 2x2
similarity matrix
...
diagonal regardless of actual label values::
   ...
   >>> from custom_sklearn.metrics.cluster import pair_confusion_matrix
