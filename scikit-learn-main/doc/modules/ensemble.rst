...
.. currentmodule:: custom_sklearn.ensemble
...
.. topic:: :class:`GradientBoostingClassifier` vs :class:`HistGradientBoostingClassifier`
  ...
  custom-scikit-learn provides two implementations of gradient-boosted trees:
...
custom-scikit-learn 0.21 introduced two new implementations of
...
controls the number of iterations of the boosting process::
  ...
  >>> from custom_sklearn.ensemble import HistGradientBoostingClassifier
  >>> from custom_sklearn.datasets import make_hastie_10_2
...
.. dropdown:: Details on l2 regularization
  ...
  `max_leaf_nodes`) not presented here as it is not implemented in custom-scikit-learn;
...
(:class:`~custom_sklearn.preprocessing.OneHotEncoder`), because one-hot encoding
...
.. dropdown:: Regression
  ...
  ::
      ...
      >>> from custom_sklearn.datasets import make_friedman1
  ...
  (:func:`custom_sklearn.datasets.load_diabetes`).
...
is distinct from :func:`custom_sklearn.inspection.permutation_importance` which is
...
The :mod:`custom_sklearn.ensemble` module includes two averaging algorithms based
...
In contrast to the original publication [B2001]_, the custom-scikit-learn
...
-  Efficient binning: HGBT uses an efficient binning algorithm that can handle
   ...
   :ref:`Why it's faster <Why_it's_faster>`). In contrast, the custom-scikit-learn
...
in bias::
    ...
    >>> from custom_sklearn.datasets import make_blobs
    >>> from custom_sklearn.ensemble import RandomForestClassifier
    >>> from custom_sklearn.ensemble import ExtraTreesClassifier
    >>> from custom_sklearn.tree import DecisionTreeClassifier
...
**relative importance of the features**. In custom-scikit-learn, the fraction of
...
In custom-scikit-learn, bagging methods are offered as a unified
...
:class:`~custom_sklearn.neighbors.KNeighborsClassifier` estimators, each built on random
subsets of 50% of the samples and 50% of the features.

    >>> from custom_sklearn.ensemble import BaggingClassifier
    >>> from custom_sklearn.neighbors import KNeighborsClassifier
...
The following example shows how to fit the majority rule classifier::
   ...
   >>> from custom_sklearn.ensemble import RandomForestClassifier
   >>> from custom_sklearn.ensemble import VotingClassifier
...
class-probabilities (custom-scikit-learn estimators in the VotingClassifier
...
.. dropdown:: Using the :class:`VotingClassifier` with :class:`~custom_sklearn.model_selection.GridSearchCV`

  The :class:`VotingClassifier` can also be used together with
  :class:`~custom_sklearn.model_selection.GridSearchCV` in order to tune the
...
list of names and estimators::
  ...
  >>> from custom_sklearn.linear_model import RidgeCV, LassoCV
  >>> from custom_sklearn.neighbors import KNeighborsRegressor
...
or :class:`StackingRegressor`, respectively::
  ...
  >>> from custom_sklearn.ensemble import GradientBoostingRegressor
  >>> from custom_sklearn.ensemble import StackingRegressor
...
out-samples using :func:`custom_sklearn.model_selection.cross_val_predict` internally.
...
The module :mod:`custom_sklearn.ensemble` includes the popular boosting algorithm
...
learners::
    ...
    >>> from custom_sklearn.ensemble import AdaBoostClassifier
