...
The :mod:`custom_sklearn.metrics.pairwise` submodule implements utilities to evaluate
...
.. currentmodule:: custom_sklearn.metrics.pairwise
...
(Note that the tf-idf functionality in ``custom_sklearn.feature_extraction.text``
...
:class:`~custom_sklearn.svm.SVC` with ``kernel="precomputed"``::
