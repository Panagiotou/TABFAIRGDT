...
In custom-scikit-learn they are passed as arguments to the constructor of the
...
- an estimator (regressor or classifier such as ``custom_sklearn.svm.SVC()``);
- a parameter space;
...
custom-scikit-learn: for given values, :class:`GridSearchCV` exhaustively considers
...
- See :ref:`sphx_glr_auto_examples_model_selection_plot_grid_search_text_feature_extraction.py` for an example
  ...
  net or L2 penalty) using a :class:`~custom_sklearn.pipeline.Pipeline` instance.
...
.. warning::
    ...
    using ``np.random.set_state``. However, beginning custom-scikit-learn 0.18,
    the :mod:`custom_sklearn.model_selection` module sets the random state provided
...
variable that is log-uniformly distributed between ``1e0`` and ``1e3``::
  ...
  from custom_sklearn.utils.fixes import loguniform
...
custom-scikit-learn also provides the :class:`HalvingGridSearchCV` and
...
need to explicitly import ``enable_halving_search_cv``::
  ...
  >>> from custom_sklearn.model_selection import HalvingGridSearchCV
...
:func:`custom_sklearn.metrics.accuracy_score` for classification and
:func:`custom_sklearn.metrics.r2_score` for regression.  For some applications,
...
:class:`~custom_sklearn.ensemble.VotingClassifier` or
:class:`~custom_sklearn.calibration.CalibratedClassifierCV` using a dedicated
``<estimator>__<parameter>`` syntax::
  ...
  >>> from custom_sklearn.ensemble import RandomForestClassifier
  >>> from custom_sklearn.datasets import make_moons
  >>> X, y = make_moons()
