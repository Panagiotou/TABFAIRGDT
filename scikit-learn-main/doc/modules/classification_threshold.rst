...
When it comes to the custom-scikit-learn API, the first point is addressed providing scores
...
classification in custom-scikit-learn, class labels predictions are obtained by hard-coded
...
:class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` tunes this threshold using
...
One way to tune the threshold is by maximizing a pre-defined custom-scikit-learn metric. These
metrics can be found by calling the function :func:`~custom_sklearn.metrics.get_scorer_names`.
...
.. note::
    ...
    :func:`~custom_sklearn.metrics.make_scorer`. Refer to :ref:`scoring` to get
    ...
    :func:`~custom_sklearn.metrics.f1_score`::
        ...
        >>> from custom_sklearn.metrics import make_scorer, f1_score
...
By default :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` uses a 5-fold
...
:class:`~custom_sklearn.model_selection.FixedThresholdClassifier`.
