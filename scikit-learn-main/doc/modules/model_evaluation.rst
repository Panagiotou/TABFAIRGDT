...
* **Metric functions**: The :mod:`custom_sklearn.metrics` module implements functions
...
Usage examples:
    ...
    >>> from custom_sklearn import svm, datasets
...
.. note::
    ...
    :func:`~custom_sklearn.metrics.get_scorer_names`.

.. currentmodule:: custom_sklearn.metrics
...
the :func:`fbeta_score` function::
    ...
    >>> from custom_sklearn.metrics import fbeta_score, make_scorer
...
The module :mod:`custom_sklearn.metrics` also exposes a set of simple functions
...
.. dropdown:: Custom scorer objects
  ...
  * any additional parameters, such as ``beta`` or ``labels`` in :func:`f1_score`.
  ...
  ``greater_is_better`` parameter::
      ...
      >>> X = [[1], [1]]
      >>> y = [0, 1]
      >>> from custom_sklearn.dummy import DummyClassifier
...
custom-scikit-learn also permits evaluation of multiple metrics in ``GridSearchCV``,
...
- As a ``dict`` mapping the scorer name to the scoring function::
      ...
      >>> from custom_sklearn.metrics import make_scorer
...
- As a callable that returns a dictionary of scores::
    ...
    >>> from custom_sklearn.metrics import confusion_matrix
...
The :mod:`custom_sklearn.metrics` module implements several loss, score, and utility
...
`indicator function <https://en.wikipedia.org/wiki/Indicator_function>`_.
  ...
  >>> import numpy as np
  >>> from custom_sklearn.metrics import top_k_accuracy_score
...
and not for more than two annotators.
  ...
  >>> from custom_sklearn.metrics import cohen_kappa_score
  >>> y_true = [2, 0, 2, 2, 0, 1]
  >>> y_pred = [0, 0, 2, 2, 0, 2]
...
and inferred labels::
   ...
   >>> from custom_sklearn.metrics import classification_report
   >>> y_true = [0, 1, 2, 2, 0]
   >>> y_pred = [0, 0, 2, 1, 0]
...
Please refer to the note below for more information. ::
  ...
  >>> from custom_sklearn.metrics import hamming_loss
  >>> y_pred = [1, 2, 3, 4]
  >>> y_true = [2, 2, 3, 4]
...
To avoid division by zero when precision and recall are zero, custom-scikit-learn calculates F-measure with this
...
Here are some small examples in binary classification::
  ...
  >>> import numpy as np
  >>> from custom_sklearn.metrics import precision_recall_curve
  >>> from custom_sklearn.metrics import average_precision_score
...
In the binary case::
  ...
  >>> import numpy as np
  >>> from custom_sklearn.metrics import jaccard_score
...
with a svm classifier in a binary class problem::
  ...
  >>> from custom_sklearn.metrics import hinge_loss
  >>> X = [[0], [1]]
  >>> y = [-1, 1]
...
method.
    ...
    >>> from custom_sklearn.metrics import log_loss
    >>> y_true = [0, 0, 1, 1]
...
function:
    ...
    >>> from custom_sklearn.metrics import matthews_corrcoef
    >>> y_true = [+1, +1, +1, -1]
    >>> y_pred = [+1, -1, +1, +1]
...
:term:`multilabel indicator matrix` input::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import multilabel_confusion_matrix
...
function::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import roc_curve
    >>> y = np.array([1, 1, 2, 2])
...
Therefore, the `y_score` parameter is of size (n_samples,).
  ...
  >>> from custom_sklearn.datasets import load_breast_cancer
  >>> from custom_sklearn.linear_model import LogisticRegression
  >>> from custom_sklearn.metrics import roc_auc_score
...
class with the greater label for each output.
  ...
  >>> from custom_sklearn.datasets import make_multilabel_classification
  >>> from custom_sklearn.multioutput import MultiOutputClassifier
...
And the decision values do not require such processing.
  ...
  >>> from custom_sklearn.linear_model import RidgeClassifierCV
...
loss can also be computed as :math:`zero-one loss = 1 - accuracy`.
  ...
  >>> from custom_sklearn.metrics import zero_one_loss
  >>> y_pred = [1, 2, 3, 4]
  >>> y_true = [2, 2, 3, 4]
...
Here is a small example of usage of this function::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import brier_score_loss
...
.. dropdown:: D2 log loss score
  ...
  Here are some usage examples of the :func:`d2_log_loss_score` function::
    ...
    >>> from custom_sklearn.metrics import d2_log_loss_score
    >>> y_true = [1, 1, 2, 3]
    >>> y_pred = [
    ...    [0.5, 0.25, 0.25],
    ...    [0.5, 0.25, 0.25],
...
Here is a small example of usage of this function::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import coverage_error
...
Here is a small example of usage of this function::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import label_ranking_average_precision_score
    >>> y_true = np.array([[1, 0, 0], [0, 0, 1]])
    >>> y_score = np.array([[0.75, 0.5, 1], [1, 0.2, 0.1]])
...
Here is a small example of usage of this function::
    ...
    >>> import numpy as np
    >>> from custom_sklearn.metrics import label_ranking_loss
    >>> y_true = np.array([[1, 0, 0], [0, 0, 1]])
    >>> y_score = np.array([[0.75, 0.5, 1], [1, 0.2, 0.1]])
...
(NDCG) are ranking metrics implemented in :func:`~custom_sklearn.metrics.dcg_score`
and :func:`~custom_sklearn.metrics.ndcg_score` ; they compare a predicted order to
...
Here is a small example of usage of the :func:`r2_score` function::
  ...
  >>> from custom_sklearn.metrics import r2_score
  >>> y_true = [3, -0.5, 2, 7]
  >>> y_pred = [2.5, 0.0, 2, 8]
...
Here is a small example of usage of the :func:`mean_absolute_error` function::
  ...
  >>> from custom_sklearn.metrics import mean_absolute_error
  >>> y_true = [3, -0.5, 2, 7]
  >>> y_pred = [2.5, 0.0, 2, 8]
...
function::
  ...
  >>> from custom_sklearn.metrics import mean_squared_log_error
  >>> y_true = [3, 5, 2.5, 7]
  >>> y_pred = [2.5, 5, 4, 8]
...
function::
  ...
  >>> from custom_sklearn.metrics import mean_absolute_percentage_error
  >>> y_true = [1, 10, 1e6]
  >>> y_pred = [0.9, 15, 1.2e6]
...
function::
  ...
  >>> from custom_sklearn.metrics import median_absolute_error
  >>> y_true = [3, -0.5, 2, 7]
  >>> y_pred = [2.5, 0.0, 2, 8]
...
Here is a small example of usage of the :func:`max_error` function::
  ...
  >>> from custom_sklearn.metrics import max_error
  >>> y_true = [3, 2, 7, 1]
  >>> y_pred = [9, 2, 7, 1]
...
function::
    ...
    >>> from custom_sklearn.metrics import explained_variance_score
    >>> y_true = [3, -0.5, 2, 7]
    >>> y_pred = [2.5, 0.0, 2, 8]
...
prediction difference of the second point,::
    ...
    >>> from custom_sklearn.metrics import mean_tweedie_deviance
...
Here is a small example of usage of the :func:`mean_pinball_loss` function::
  ...
  >>> from custom_sklearn.metrics import mean_pinball_loss
  >>> y_true = [1, 2, 3]
...
quantile regressor via cross-validation:
  ...
  >>> from custom_sklearn.model_selection import cross_val_score
  >>> from custom_sklearn.ensemble import GradientBoostingRegressor
  >>>
...
.. dropdown:: D² Tweedie score
  ...
  A scorer object with a specific choice of ``power`` can be built by::
    ...
    >>> from custom_sklearn.metrics import d2_tweedie_score, make_scorer
...
.. dropdown:: D² pinball score
  ...
  A scorer object with a specific choice of ``alpha`` can be built by::
    ...
    >>> from custom_sklearn.metrics import d2_pinball_score, make_scorer
...
.. dropdown:: D² absolute error score
  ...
  Here are some usage examples of the :func:`d2_absolute_error_score` function::
    ...
    >>> from custom_sklearn.metrics import d2_absolute_error_score
    >>> y_true = [3, -0.5, 2, 7]
    >>> y_pred = [2.5, 0.0, 2, 8]
...
Among methods to assess the quality of regression models, custom-scikit-learn provides
the :class:`~custom_sklearn.metrics.PredictionErrorDisplay` class. It allows to
...
of `y` given `X`, e.g. :class:`~custom_sklearn.linear_model.QuantileRegressor`
...
:class:`~custom_sklearn.linear_model.LinearRegression` and
:class:`~custom_sklearn.linear_model.Ridge`), we can use this plot to check
...
* See :ref:`sphx_glr_auto_examples_compose_plot_transformed_target.py` for
  an example on how to use :class:`~custom_sklearn.metrics.PredictionErrorDisplay`
...
.. currentmodule:: custom_sklearn.dummy
