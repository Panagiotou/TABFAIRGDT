...
- Improve narrative documentation and consistency in
  :mod:`custom_sklearn.metrics` for regression and classification metrics
  by `Arnaud Joly`_.

- Fixed a bug in :class:`custom_sklearn.svm.SVC` when using csr-matrices with
...
- The module ``custom_sklearn.linear_model.sparse`` is gone. Sparse matrix support
  was already integrated into the "regular" linear models.

- `custom_sklearn.metrics.mean_square_error`, which incorrectly returned the
...
- Renamed function `custom_sklearn.metrics.zero_one` to
  `custom_sklearn.metrics.zero_one_loss`. Be aware that the default behavior
  in `custom_sklearn.metrics.zero_one_loss` is different from
  `custom_sklearn.metrics.zero_one`: ``normalize=False`` is changed to
  ``normalize=True``.
