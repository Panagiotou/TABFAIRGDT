...
.. warning::
    ...
    Version 0.20 is the last version of custom-scikit-learn to support Python 2.7 and Python 3.4.
    custom-scikit-learn 0.21 will require Python 3.5 or higher.
...
- |Efficiency| :func:`custom_sklearn.datasets.fetch_openml` now loads data by
...
- |Fix| add support for non-numeric data in
  :class:`custom_sklearn.impute.MissingIndicator` which was not supported while
  :class:`custom_sklearn.impute.SimpleImputer` was supporting this for some
...
- :mod:`custom_sklearn.neighbors` when ``metric=='jaccard'`` (bug fix)
...
- |Fix| Fixed `custom_sklearn.neighbors.DistanceMetric` jaccard distance
...
- |Fix| When using site joblib by setting the environment variable
  `custom_sklearn_SITE_JOBLIB`, added compatibility with joblib 0.11 in addition
...
- |Fix| Make sure to avoid raising ``FutureWarning`` when calling
  ...
  instead of generator expressions in many locations of the custom-scikit-learn
  code base). :issue:`12467` by :user:`Olivier Grisel <ogrisel>`.

- |API| Removed all mentions of ``custom_sklearn.externals.joblib``, and deprecated
  joblib methods exposed in ``custom_sklearn.utils``, except for
  :func:`utils.parallel_backend` and :func:`utils.register_parallel_backend`,
  which allow users to configure parallel computation in custom-scikit-learn.
  ...
  unvendoring joblib in future version of custom-scikit-learn.
...
the custom-scikit-learn library, and improvements to the documentation and examples.
...
NaNs, and data containing NaNs can be transformed. The new :mod:`custom_sklearn.impute`
...
- |Feature| Add ``filename`` attribute to :mod:`custom_sklearn.datasets` that have a CSV file.
...
- |API| Deprecated `custom_sklearn.datasets.fetch_mldata` to be removed in
...
- |Efficiency| Memory usage improvement for `_class_means` and
  `_class_cov` in :mod:`custom_sklearn.discriminant_analysis`. :issue:`10898` by
  :user:`Nanxin Chen <bobchennan>`.


:mod:`custom_sklearn.dummy`
....................
...
- |Fix| Fixed a bug in :class:`custom_sklearn.linear_model.Lasso`
  where the coefficient had wrong shape when ``fit_intercept=False``.
  :issue:`10687` by :user:`Martin Hahn <martin-hahn>`.

- |Fix| Fixed a bug in :func:`custom_sklearn.linear_model.LogisticRegression` where the
...
- |Efficiency| :mod:`custom_sklearn.neighbors` query methods are now more
...
- |Feature| The ``transform`` method of :class:`custom_sklearn.preprocessing.MultiLabelBinarizer`
...
- |Feature| The version of :mod:`joblib` bundled with custom-scikit-learn is now 0.12.
...
- |Feature| An environment variable to use the site joblib instead of the
  ...
  is now exposed in :mod:`custom_sklearn.utils`.
...
- |Feature| A utility method :func:`custom_sklearn.show_versions()` was added to
...
- |Fix| Fixed a bug where calling :func:`custom_sklearn.base.clone` was not thread
...
- Checks for transformers now apply if the estimator implements
  ...
  :class:`custom_sklearn.base.TransformerMixin`. :issue:`10474` by `Joel Nothman`_.
