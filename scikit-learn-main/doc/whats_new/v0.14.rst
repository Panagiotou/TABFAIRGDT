...
- Added :ref:`biclustering <biclustering>` algorithms
  (`custom_sklearn.cluster.bicluster.SpectralCoclustering` and
  `custom_sklearn.cluster.bicluster.SpectralBiclustering`), data
  generation methods (:func:`custom_sklearn.datasets.make_biclusters` and
  :func:`custom_sklearn.datasets.make_checkerboard`), and scoring metrics
  (:func:`custom_sklearn.metrics.consensus_score`). By `Kemal Eren`_.
...
- Fixed `custom_sklearn.linear_model.stochastic_gradient.py` L2 regularization
...
- Added an interactive version of `Andreas Müller`_'s
  `Machine Learning Cheat Sheet (for custom-scikit-learn)
...
- `grid_search.GridSearchCV` and
  ...
  and `Lars Buitinck`_.
  Passing a function from :mod:`custom_sklearn.metrics` as ``score_func`` is
  deprecated.
...
- The new estimator :class:`custom_sklearn.decomposition.TruncatedSVD`
...
- The default number of components for
  `custom_sklearn.decomposition.RandomizedPCA` is now correctly documented
  to be ``n_features``. This was the default behavior, so programs using it
  will continue to work as they did.

- :class:`custom_sklearn.cluster.KMeans` now fits several orders of magnitude
...
- Verbose output in `custom_sklearn.ensemble.gradient_boosting` now uses
  a column format and prints progress in decreasing frequency.
  It also shows the remaining time. By `Peter Prettenhofer`_.

- `custom_sklearn.ensemble.gradient_boosting` provides out-of-bag improvement
  `oob_improvement_`
...
- Fixed a bug in `custom_sklearn.covariance.GraphLassoCV`: the
  'alphas' parameter now works as expected when given a list of
  values. By Philippe Gervais.

- Fixed an important bug in `custom_sklearn.covariance.GraphLassoCV`
...
- Out-of-core learning support for discrete naive Bayes classifiers
  :class:`custom_sklearn.naive_bayes.MultinomialNB` and
  :class:`custom_sklearn.naive_bayes.BernoulliNB` by adding the ``partial_fit``
...
- Better input and error handling in the :mod:`custom_sklearn.metrics` module by
  `Arnaud Joly`_ and `Joel Nothman`_.

- Speed optimization of the `hmm` module by :user:`Mikhail Korobov <kmike>`

- Significant speed improvements for :class:`custom_sklearn.cluster.DBSCAN`
...
- Testing custom-scikit-learn with ``custom_sklearn.test()`` is deprecated. Use
  ``nosetests custom_sklearn`` from the command line.
...
- :class:`custom_sklearn.neighbors.BallTree` has been refactored, and a
  :class:`custom_sklearn.neighbors.KDTree` has been
...
- Support for scipy.spatial.cKDTree within neighbors queries has been
  ...
  :class:`custom_sklearn.neighbors.KDTree` class.

- :class:`custom_sklearn.neighbors.KernelDensity` has been added, which performs
  efficient kernel density estimation with a variety of kernels.

- :class:`custom_sklearn.decomposition.KernelPCA` now always returns output with
...
- ``gcv_mode="auto"`` no longer tries to perform SVD on a densified
  sparse matrix in :class:`custom_sklearn.linear_model.RidgeCV`.

- Sparse matrix support in `custom_sklearn.decomposition.RandomizedPCA`
...
- Attribute ``oob_score_`` in :class:`custom_sklearn.ensemble.GradientBoostingRegressor`
  and :class:`custom_sklearn.ensemble.GradientBoostingClassifier`
...
- :class:`custom_sklearn.preprocessing.StandardScaler` now converts integer input
  to float, and raises a warning. Previously it rounded for dense integer
  input.

- :class:`custom_sklearn.multiclass.OneVsRestClassifier` now has a
