...
The 0.24.0 custom-scikit-learn wheels were not working with MacOS <1.15 due to
`libomp`. The version of `libomp` used to build the wheels was too recent for
older macOS versions. This issue has been fixed for 0.24.1 custom-scikit-learn wheels.
custom-scikit-learn wheels published on PyPI.org now officially support macOS 10.13
and later.
...
- |Fix| Fix :class:`decomposition.SparseCoder` such that it follows
  custom-scikit-learn API and support cloning. The attribute `components_` is
...
- |Efficiency| :class:`impute.SimpleImputer` is now faster with `object` dtype array.
  when `strategy='most_frequent'` in :class:`~custom_sklearn.impute.SimpleImputer`.
...
- |Fix| Raise error on
  :meth:`custom_sklearn.preprocessing.OneHotEncoder.inverse_transform`
