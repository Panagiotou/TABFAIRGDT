...
- The old ``scikits.learn`` package has disappeared; all code should import
  from ``custom_sklearn`` instead, which was introduced in 0.9.
...
- Added BIC/AIC model selection to classical :ref:`gmm` and unified
  the API with the remainder of custom-scikit-learn, by `Bertrand Thirion`_

- Added `custom_sklearn.cross_validation.StratifiedShuffleSplit`, which is
  a `custom_sklearn.cross_validation.ShuffleSplit` with balanced splits,
  by Yannick Schwartz.

- :class:`~custom_sklearn.neighbors.NearestCentroid` classifier added, along with a
...
- Python 2.5 compatibility was dropped; the minimum Python version needed
  to use custom-scikit-learn is now 2.6.
...
- Silhouette Coefficient cluster analysis evaluation metric added as
  :func:`~custom_sklearn.metrics.silhouette_score` by Robert Layton.
...
- Adjusted Mutual Information metric added as
  :func:`~custom_sklearn.metrics.adjusted_mutual_info_score` by Robert Layton.
...
- Improved documentation for developers and for the :mod:`custom_sklearn.utils`
  module, by `Jake Vanderplas`_.

- Vectorized 20newsgroups dataset loader
  (:func:`~custom_sklearn.datasets.fetch_20newsgroups_vectorized`) by
  `Mathieu Blondel`_.
...
- Make :func:`~custom_sklearn.preprocessing.scale` and
  `custom_sklearn.preprocessing.Scaler` work on sparse matrices by
  `Olivier Grisel`_
...
- `custom_sklearn.cross_validation.ShuffleSplit` can subsample the train
...
Here are the code migration instructions when upgrading from custom-scikit-learn
version 0.9:
...
- Some estimators that may overwrite their inputs to save memory previously
  ...
  This particularly affects some of the estimators in :mod:`~custom_sklearn.linear_model`.
  The default behavior is still to copy everything passed in.

- The SVMlight dataset loader :func:`~custom_sklearn.datasets.load_svmlight_file` no
...
- Cluster evaluation metrics in :mod:`~custom_sklearn.metrics.cluster` have been refactored
...
- The functions used for sparse coding, ``sparse_encode`` and
  ...
  :func:`~custom_sklearn.decomposition.sparse_encode`, and the shapes of the arrays
...
- Fixed an off-by-one error in the SVMlight/LibSVM file format handling;
  files generated using :func:`~custom_sklearn.datasets.dump_svmlight_file` should be
  re-generated. (They should continue to work, but accidentally had one
  extra column of zeros prepended.)

- ``BaseDictionaryLearning`` class replaced by ``SparseCodingMixin``.

- `custom_sklearn.utils.extmath.fast_svd` has been renamed
  :func:`~custom_sklearn.utils.extmath.randomized_svd` and the default
...
The following people contributed to custom-scikit-learn since last release:
...
custom-scikit-learn 0.9 was released on September 2011, three months after the 0.8
...
- The ``scikits.learn`` package was renamed ``custom_sklearn``. There is
  still a ``scikits.learn`` package alias for backward compatibility.

  Third-party projects with a dependency on custom-scikit-learn 0.9+ should
  upgrade their codebase. For instance, under Linux / MacOSX just run
  (make a backup first!)::

      find -name "*.py" | xargs sed -i 's/\bscikits.learn\b/custom_sklearn/g'
...
- The ``score_func`` argument of the
  ``custom_sklearn.cross_validation.cross_val_score`` function is now expected
...
- The ``custom_sklearn.hmm`` has been marked as orphaned: it will be removed
  from custom-scikit-learn in version 0.11 unless someone steps up to
  contribute documentation, examples and fix lurking numerical
  stability issues.

- ``custom_sklearn.neighbors`` has been made into a submodule.  The two previously
...
- ``custom_sklearn.ball_tree.BallTree`` has been moved to
  ``custom_sklearn.neighbors.BallTree``.  Using the former will generate a warning.

- ``custom_sklearn.linear_model.LARS()`` and related classes (LassoLARS,
  LassoLARSCV, etc.) have been renamed to
  ``custom_sklearn.linear_model.Lars()``.

- All distance metrics and kernels in ``custom_sklearn.metrics.pairwise`` now have a Y
...
- ``custom_sklearn.metrics.pairwise.l1_distance`` is now called ``manhattan_distance``,
...
custom-scikit-learn 0.8 was released on May 2011, one month after the first
"international" `custom-scikit-learn coding sprint
<https://github.com/custom-scikit-learn/custom-scikit-learn/wiki/Upcoming-events>`_ and is
...
custom-scikit-learn 0.7 was released in March 2011, roughly three months
...
- Refactoring of `neighbors.NeighborsClassifier` and
  ...
  `notes_neighbors
  <https://github.com/custom-scikit-learn/custom-scikit-learn/wiki/Neighbors-working-notes>`_ for more information [`Fabian Pedregosa`_].
...
custom-scikit-learn 0.6 was released on December 2010. It is marked by the
...
- New `stochastic gradient
  <https://custom-scikit-learn.org/stable/modules/sgd.html>`_ descent
...
- Improved sparse matrix support, both in main classes
  ...
  custom_sklearn.svm.sparse and custom_sklearn.linear_model.sparse.
...
- New FastICA algorithm in module custom_sklearn.fastica
...
- Improved documentation for many modules, now separating
  ...
  <https://custom-scikit-learn.org/stable/modules/svm.html>`_ and the
  complete `class reference
  <https://custom-scikit-learn.org/stable/modules/classes.html>`_.
...
- Many more examples. `See here
  <https://custom-scikit-learn.org/stable/auto_examples/index.html>`_
...
- Joblib is now a dependency of this package, although it is
  shipped with (custom_sklearn.externals.joblib).
