...
- |Fix| :class:`feature_extraction.text.CountVectorizer` and
  ...
  with the new custom-scikit-learn version or to manually clear the `stop_words_`
...
- :func:`custom_sklearn.metrics.r2_score` now supports Array API compliant inputs.
...
From custom-scikit-learn 1.5 onwards, Meson is the main supported way to build
custom-scikit-learn, see :ref:`Building from source <install_bleeding_edge>` for more
details.

Unless we discover a major blocker, setuptools support will be dropped in
custom-scikit-learn 1.6. The 1.5.x releases will support building custom-scikit-learn with
setuptools.

Meson support for building custom-scikit-learn was added in :pr:`28040` by
...
- |Fix| :class:`cluster.HDBSCAN` now supports all metrics supported by
  :func:`custom_sklearn.metrics.pairwise_distances` when `algorithm="brute"` or `"auto"`.
...
- |Feature| :func:`custom_sklearn.metrics.check_scoring` now returns a multi-metric scorer
...
Mahmood, Salim Dohri, Sandip Dutta, SarahRemus, custom-scikit-learn-bot, Shaharyar 
