...
- |Fix| All :class:`custom_sklearn.metrics.DistanceMetric` subclasses now correctly support
...
- |Fix| All `custom_sklearn.metrics.MinkowskiDistance` now accepts a weight
...
- |Fix| Compute `y_std` properly with multi-target in
  :class:`custom_sklearn.gaussian_process.GaussianProcessRegressor` allowing
...
- |Fix| Solve a bug in ``custom_sklearn.utils.metaestimators.if_delegate_has_method``
...
Version 1.0.0 of custom-scikit-learn requires python 3.7+, numpy 1.14.6+ and
...
<https://custom-scikit-learn-enhancement-proposals.readthedocs.io/en/latest/slep009/proposal.html>`_
for more details.
...
- |Enhancement| :class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` and
  :class:`~custom_sklearn.ensemble.HistGradientBoostingRegressor` take cgroups quotas
...
- |Enhancement| :class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` and
  :class:`~custom_sklearn.ensemble.HistGradientBoostingRegressor` are no longer
...
- |Fix| Do not allow to compute out-of-bag (OOB) score in
  ...
  since custom-scikit-learn does not provide any metric supporting this type of
...
- |API|: The parameter ``normalize`` of :class:`linear_model.LinearRegression`
  ...
  :class:`~custom_sklearn.pipeline.Pipeline` with ``LinearModel`` (where
...
- |API| Keyword validation has moved from `__init__` and `set_params` to `fit`
  for the following estimators conforming to custom-scikit-learn's conventions:
...
- |Enhancement| Deprecated the default value of the `random_state=0` in
  :func:`~custom_sklearn.utils.extmath.randomized_svd`. Starting in 1.2,
...
- |Enhancement| :func:`utils.validation.check_is_fitted` now uses
  ``__custom_sklearn_is_fitted__`` if available, instead of checking for attributes
...
- |Fix| Support for `np.matrix` is deprecated in
  :func:`~custom_sklearn.utils.check_array` in 1.0 and will raise a `TypeError` in
  1.2. :pr:`20165` by `Thomas Fan`_.
