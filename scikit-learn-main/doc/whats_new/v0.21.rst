...
- |Fix| :func:`datasets.fetch_california_housing`,
  ...
  data was persisted using the deprecated ``custom_sklearn.externals.joblib``. This
...
`custom_sklearn.utils.sparsefuncs`
...........................
...
- :class:`custom_sklearn.feature_extraction.text.HashingVectorizer`,
  :class:`custom_sklearn.feature_extraction.text.TfidfVectorizer`, and
  :class:`custom_sklearn.feature_extraction.text.CountVectorizer` |Fix|
...
- |MajorFeature| Add two new implementations of
  ...
  need to explicitly import ``enable_hist_gradient_boosting``::
    ...
    >>> # now you can import normally from custom_sklearn.ensemble
  ...
  .. note::
      ...
      anymore and you don't need to use `from custom_sklearn.experimental import
...
- |Efficiency| :class:`custom_sklearn.ensemble.GradientBoostingClassifier` and
  :class:`custom_sklearn.ensemble.GradientBoostingRegressor` now keep the
...
- |Fix| Fixed a bug in :class:`ensemble.GradientBoostingClassifier` and
  ...
  custom-scikit-learn estimators as the initial estimator. Also added support of
...
- |Fix| `ensemble.partial_dependence` (and consequently the new
  version :func:`custom_sklearn.inspection.partial_dependence`) now takes sample
...
- |API| `ensemble.partial_dependence` and
  ...
  :func:`inspection.partial_dependence<custom_sklearn.inspection.partial_dependence>`
  and
  `inspection.plot_partial_dependence<custom_sklearn.inspection.plot_partial_dependence>`.
...
- |Fix| If ``input='file'`` or ``input='filename'``, and a callable is given as
  the ``analyzer``, :class:`custom_sklearn.feature_extraction.text.HashingVectorizer`,
  :class:`custom_sklearn.feature_extraction.text.TfidfVectorizer`, and
  :class:`custom_sklearn.feature_extraction.text.CountVectorizer` now read the data
...
- |API| The default value of `copy` in :func:`preprocessing.quantile_transform`
  ...
  :mod:`custom_sklearn.preprocessing` and prevent unexpected side effects by modifying
...
- |MajorFeature| The `__repr__()` method of all estimators (used when calling
  ...
  :func:`custom_sklearn.set_config`. :pr:`11705` by :user:`Nicolas Hug
  <NicolasHug>`.
...
- |Enhancement| Joblib is no longer vendored in custom-scikit-learn, and becomes a
