...
- |API| The `StreamHandler` was removed from `custom_sklearn.logger` to avoid
...
- |MajorFeature| Estimators can now be displayed with a rich html
  ...
  `display='diagram'` in :func:`~custom_sklearn.set_config`. The raw html can be
...
- |Enhancement| :func:`utils.check_array` supports pandas'
  ...
  all :mod:`custom_sklearn.preprocessing` transformers that accept numeric inputs with
...
- |MajorFeature| Adds a HTML representation of estimators to be shown in
  ...
  `display` option in :func:`custom_sklearn.set_config`. :pr:`14180` by
  `Thomas Fan`_.

- |Enhancement| ``custom-scikit-learn`` now works with ``mypy`` without errors.
...
- |API| Estimators now have a `requires_y` tags which is False by default
  except for estimators that inherit from `~custom_sklearn.base.RegressorMixin` or
  `~custom_sklearn.base.ClassifierMixin`. This tag is used to ensure that a proper
...
- |API| The default setting `print_changed_only` has been changed from False
  ...
  `custom_sklearn.set_config(print_changed_only=False)`. Also, note that it is
