...
.. warning::
    ...
    custom-scikit-learn 0.18 is the last major release of custom-scikit-learn to support Python 2.6.
    Later versions of custom-scikit-learn will require Python 2.7 or above.
...
- Fix a bug where :class:`custom_sklearn.feature_selection.SelectFdr` did not
  exactly implement Benjamini-Hochberg procedure. It formerly may have
  selected fewer features than it should.
  :issue:`7490` by :user:`Peng Meng <mpjlu>`.

- :class:`custom_sklearn.manifold.LocallyLinearEmbedding` now correctly handles
...
- :class:`custom_sklearn.linear_model.LogisticRegressionCV` now correctly handles
  string labels. :issue:`5874` by `Raghav RV`_.

- Fixed a bug where :func:`custom_sklearn.model_selection.train_test_split` raised
  an error when ``stratify`` is a list of string labels. :issue:`7593` by
  `Raghav RV`_.

- Fixed a bug where :class:`custom_sklearn.model_selection.GridSearchCV` and
  :class:`custom_sklearn.model_selection.RandomizedSearchCV` were not pickleable
  because of a pickling bug in ``np.ma.MaskedArray``. :issue:`7594` by
  `Raghav RV`_.

- All cross-validation utilities in :mod:`custom_sklearn.model_selection` now
  ...
  The :class:`custom_sklearn.model_selection.GridSearchCV` will cross-validate each
...
- **The model_selection module**
  ...
  The new module :mod:`custom_sklearn.model_selection`, which groups together the
  functionalities of formerly `custom_sklearn.cross_validation`,
  `custom_sklearn.grid_search` and `custom_sklearn.learning_curve`, introduces new
...
- **Data-independent CV splitters enabling nested cross-validation**
  ...
  :mod:`custom_sklearn.model_selection`, are no longer initialized with any
...
- The cross-validation iterators are replaced by cross-validation splitters
  available from :mod:`custom_sklearn.model_selection`, allowing for nested
...
- Added support for substituting or disabling :class:`pipeline.Pipeline`
  ...
  interface that powers `custom_sklearn.grid_search`.
...
- When unpickling a custom-scikit-learn estimator in a different version than the one
...
- Fix in :class:`custom_sklearn.model_selection.StratifiedShuffleSplit` to
...
- The `custom_sklearn.cross_validation`, `custom_sklearn.grid_search` and
  `custom_sklearn.learning_curve` have been deprecated and the classes and
  functions have been reorganized into the :mod:`custom_sklearn.model_selection`
...
custom_sklearn-ci, SmedbergM, srvanrell, Sébastien Lerique, Taranjeet, themrmax,
