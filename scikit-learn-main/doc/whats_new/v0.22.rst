...
`Our website <https://custom-scikit-learn.org/>`_ was revamped and given a fresh
new look. :pr:`14849` by `Thomas Fan`_.

Clear definition of the public API
----------------------------------

custom-scikit-learn has a public API, and a private API.
...
<https://custom-scikit-learn.org/dev/modules/classes.html>`_ and if it can be
imported with an import path without leading underscores. For example
``custom_sklearn.pipeline.make_pipeline`` is public, while
`custom_sklearn.pipeline._name_estimators` is private.
``custom_sklearn.ensemble._gb.BaseEnsemble`` is private too because the whole `_gb`
module is private.
...
should for example use ``from custom_sklearn.cluster import Birch`` instead of
``from custom_sklearn.cluster.birch import Birch`` (in practice, ``birch.py`` has
been moved to ``_birch.py``).

.. note::

    All the tools in the public API should be documented in the `API
    Reference <https://custom-scikit-learn.org/dev/modules/classes.html>`_. If you
...
<https://github.com/custom-scikit-learn/custom-scikit-learn/issues/9250>`_ and `issue
12927 <https://github.com/custom-scikit-learn/custom-scikit-learn/issues/12927>`_.


Deprecations: using ``FutureWarning`` from now on
-------------------------------------------------

When deprecating a feature, previous versions of custom-scikit-learn used to raise
a ``DeprecationWarning``. Since the ``DeprecationWarnings`` aren't shown by
default by Python, custom-scikit-learn needed to resort to a custom warning filter
to always show the warnings. That filter would sometimes interfere
with users custom warning filters.

Starting from version 0.22, custom-scikit-learn will show ``FutureWarnings`` for
...
filter has been removed and custom-scikit-learn no longer hinders with user
...
- |Enhancement| Updated the following :mod:`custom_sklearn.feature_selection`
...
- |API| custom-scikit-learn now converts any input data structure implementing a
