...
- Custom metrics for the :mod:`custom_sklearn.neighbors` binary trees now have
  fewer constraints: they must take two 1d-arrays and return a float.
  :issue:`6288` by `Jake Vanderplas`_.

- ``algorithm='auto`` in :mod:`custom_sklearn.neighbors` estimators now chooses the most
...
- Small performance improvement to n-gram creation in
  :mod:`custom_sklearn.feature_extraction.text` by binding methods for loops and
...
- Added type checking to the ``accept_sparse`` parameter in
  :mod:`custom_sklearn.utils.validation` methods. This parameter now accepts only boolean,
...
- Fix bug where :mod:`custom_sklearn.mixture` ``sample`` methods did not return as many
...
- :func:`metrics.average_precision_score` no longer linearly
  ...
  (`#7356 <https://github.com/custom-scikit-learn/custom-scikit-learn/pull/7356>`_). By
...
- Some ``fetch_`` functions in :mod:`custom_sklearn.datasets` were ignoring the
...
- Add ``data_home`` parameter to :func:`custom_sklearn.datasets.fetch_kddcup99`.
...
- The ``decision_function`` output shape for binary classification in
  ...
  to custom-scikit-learn conventions. :issue:`9100` by `Andreas Müller`_.
...
- SciPy >= 0.13.3 and NumPy >= 1.8.2 are now the minimum supported versions
  for custom-scikit-learn. The following backported functions in
  :mod:`custom_sklearn.utils` have been removed or deprecated accordingly.
