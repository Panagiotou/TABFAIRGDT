...
- Fixed reading of Bunch pickles generated with custom-scikit-learn
  version <= 0.16. This can affect users who have already
  downloaded a dataset with custom-scikit-learn 0.16 and are loading it
  with custom-scikit-learn 0.17. See :issue:`6196` for
...
- The ``labels`` parameter to :func:`custom_sklearn.metrics.f1_score`,
  :func:`custom_sklearn.metrics.fbeta_score`,
  :func:`custom_sklearn.metrics.recall_score` and
  :func:`custom_sklearn.metrics.precision_score` has been extended.
...
- Provide an option for sparse output from
  :func:`custom_sklearn.metrics.pairwise.cosine_similarity`. By
...
- RCV1 dataset loader (:func:`custom_sklearn.datasets.fetch_rcv1`).
  By `Tom Dupre la Tour`_.

- The "Wisconsin Breast Cancer" classical two-class classification dataset
  is now included in custom-scikit-learn, available with
  :func:`datasets.load_breast_cancer`.

- Upgraded to joblib 0.9.3 to benefit from the new automatic batching of
  short tasks. This makes it possible for custom-scikit-learn to benefit from
