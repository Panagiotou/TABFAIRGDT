...
- Fixed the build under Windows when custom-scikit-learn is built with MSVC while
...
- ``nose`` is non-longer a runtime dependency to import ``custom_sklearn``, only for
...
- The ``img_to_graph`` and ``grid_tograph`` functions in
  :mod:`custom_sklearn.feature_extraction.image` now return ``np.ndarray``
...
- Speed improvement of the `custom_sklearn.ensemble.gradient_boosting` module.
  By `Gilles Louppe`_ and `Peter Prettenhofer`_.

- Various enhancements to the `custom_sklearn.ensemble.gradient_boosting`
...
- Faster `custom_sklearn.ensemble.ExtraTrees` by caching feature values.
  By `Arnaud Joly`_.
...
- The :ref:`Working With Text Data <text_data_tutorial>` tutorial
  ...
  Tutorial integration into the custom-scikit-learn documentation
  by `Jaques Grobler`_
...
- `custom_sklearn.hmm` is deprecated. Its removal is planned
