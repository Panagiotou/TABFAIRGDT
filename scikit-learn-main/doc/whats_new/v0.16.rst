...
- A discrepancy between liblinear output and custom-scikit-learn's wrappers
...
- :class:`custom_sklearn.neighbors.BallTree` and :class:`custom_sklearn.neighbors.KDTree`
...
- Correct ``partial_fit`` handling of ``class_prior`` for
  ...
  :class:`custom_sklearn.naive_bayes.BernoulliNB`. By `Trevor Stephens`_.
...
- Avoid skipping the first nearest neighbor in the methods ``radius_neighbors``,
  ...
  :class:`custom_sklearn.neighbors.NearestNeighbors` and family, when the query
...
- Nearest neighbors estimators used to take arbitrary keyword arguments
  ...
  in custom-scikit-learn 0.18; use the ``metric_params`` argument instead.
...
- Users should now supply an explicit ``average`` parameter to
  :func:`custom_sklearn.metrics.f1_score`, :func:`custom_sklearn.metrics.fbeta_score`,
  :func:`custom_sklearn.metrics.recall_score` and
  :func:`custom_sklearn.metrics.precision_score` when performing multiclass
...
- Allow ``X=None`` in the methods ``radius_neighbors``, ``kneighbors``,
  ...
  :class:`custom_sklearn.neighbors.NearestNeighbors` and family. If set to None,
