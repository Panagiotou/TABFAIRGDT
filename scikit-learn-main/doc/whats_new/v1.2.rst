...
- |Fix| Fix a bug where the current configuration was ignored in estimators using
  ...
  thread of `joblib` as :func:`custom_sklearn.get_config` used to access an empty thread
...
- |API| `utils.fixes.delayed` is deprecated in 1.2.1 and will be removed
  ...
  to ensure proper propagation of the custom-scikit-learn configuration to
  the workers.
  :pr:`25363` by :user:`Guillaume Lemaitre <glemaitre>`.
...
- |Efficiency| Low-level routines for reductions on pairwise distances
  ...
  For instance :meth:`custom_sklearn.neighbors.NearestNeighbors.kneighbors` and
  :meth:`custom_sklearn.neighbors.NearestNeighbors.radius_neighbors`
...
- |Feature| A new module exposes development tools to discover estimators (i.e.
  ...
  :func:`utils.discovery.all_functions`) in custom-scikit-learn.
