...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
custom-scikit-learn, but not scikit or SciKit nor sci-kit learn.
...
Apart from custom-scikit-learn, another popular one is `scikit-image <https://scikit-image.org/>`_.

Do you support PyPy?
^^^^^^^^^^^^^^^^^^^^

custom-scikit-learn is regularly tested and maintained to work with
...
How can I obtain permission to use the images in custom-scikit-learn for my work?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The images contained in the `custom-scikit-learn repository
<https://github.com/custom-scikit-learn/custom-scikit-learn>`_ and the images generated within
the `custom-scikit-learn documentation <https://custom-scikit-learn.org/stable/index.html>`_
can be used via the `BSD 3-Clause License
<https://github.com/custom-scikit-learn/custom-scikit-learn?tab=BSD-3-Clause-1-ov-file>`_ for
your work. Citations of custom-scikit-learn are highly encouraged and appreciated. See
:ref:`citing custom-scikit-learn <citing-custom-scikit-learn>`.
...
the design constraints of custom-scikit-learn. As a result, deep learning
and reinforcement learning are currently out of scope for what
custom-scikit-learn seeks to achieve.

You can find more information about the addition of GPU support at
`Will you add GPU support?`_.

Note that custom-scikit-learn currently implements a simple multilayer perceptron
in :mod:`custom_sklearn.neural_network`. We will only accept bug fixes for this module.
...
Will you add graphical models or sequence prediction to custom-scikit-learn?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Not in the foreseeable future.
custom-scikit-learn tries to provide a unified API for the basic tasks in machine
...
custom-scikit-learn has to offer. If we started doing arbitrary structured
learning, we'd need to redesign the whole package and the project
would likely collapse under its own weight.

There are two projects with API similar to custom-scikit-learn that
...
Why did you remove HMMs from custom-scikit-learn?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
make it both harder for the average user to install custom-scikit-learn and harder for
the developers to maintain the code.

However, since 2023, a limited but growing :ref:`list of custom-scikit-learn
estimators <array_api_supported>` can already run on GPUs if the input data is
provided as a PyTorch or CuPy array and if custom-scikit-learn has been configured to
accept such inputs as explained in :ref:`array_api`. This Array API support
allows custom-scikit-learn to run on GPUs without introducing heavy and
...
However, not all custom-scikit-learn estimators are amenable to efficiently running
on GPUs via the Array API for fundamental algorithmic reasons. For instance,
tree-based models currently implemented with Cython in custom-scikit-learn are
...
flexible extension system for custom-scikit-learn. This possibility is being
considered in the following GitHub issue (under discussion):

- https://github.com/custom-scikit-learn/custom-scikit-learn/issues/22438


Why do categorical variables need preprocessing in custom-scikit-learn, compared to other tools?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Most of custom-scikit-learn assumes data is in NumPy arrays or SciPy sparse matrices
...
Why does custom-scikit-learn not directly work with, for example, :class:`pandas.DataFrame`?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Note however that :class:`~custom_sklearn.compose.ColumnTransformer` makes it
convenient to handle heterogeneous pandas dataframes by mapping homogeneous subsets of
dataframe columns selected by name or dtype to dedicated custom-scikit-learn transformers.
Therefore :class:`~custom_sklearn.compose.ColumnTransformer` are often used in the first
step of custom-scikit-learn pipelines when dealing
...
and `imbalanced-learn <https://github.com/custom-scikit-learn-contrib/imbalanced-learn>`_.
Note that custom-scikit-learn solved for the case where ``y``
has an invertible transformation applied before training
and inverted after prediction. custom-scikit-learn intends to solve for
use cases where ``y`` should be transformed at training time
and not at test time, for resampling and similar uses, like at
`imbalanced-learn <https://github.com/custom-scikit-learn-contrib/imbalanced-learn>`_.
...
How can I contribute to custom-scikit-learn?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
of custom-scikit-learn directly regarding contributing to custom-scikit-learn.

Why is my pull request not getting any attention?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The custom-scikit-learn review process takes a significant amount of time, and
...
Beyond that, custom-scikit-learn is limited in its reviewing bandwidth; many of the
reviewers and core developers are working on custom-scikit-learn on their own time.
...
which fit well within the current API of custom-scikit-learn, that is a ``fit``,
...
in custom-scikit-learn at least in some areas.
...
Also, note that your implementation need not be in custom-scikit-learn to be used
together with custom-scikit-learn tools. You can implement your favorite algorithm
in a custom-scikit-learn compatible way, upload it to GitHub and let us know. We
will be happy to list it under :ref:`related_projects`. If you already have
a package on GitHub following the custom-scikit-learn API, you may also be
interested to look at `custom-scikit-learn-contrib
<https://custom-scikit-learn-contrib.github.io>`_.

.. _selectiveness:

Why are you so selective on what algorithms you include in custom-scikit-learn?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Using custom-scikit-learn
------------------

What's the best way to get help on custom-scikit-learn usage?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

* General machine learning questions: use `Cross Validated
  <https://stats.stackexchange.com/>`_ with the ``[machine-learning]`` tag.

* custom-scikit-learn usage questions: use `Stack Overflow
  <https://stackoverflow.com/questions/tagged/custom-scikit-learn>`_ with the
  ``[custom-scikit-learn]`` and ``[python]`` tags. You can alternatively use the `mailing list
  <https://mail.python.org/mailman/listinfo/custom-scikit-learn>`_.
...
:mod:`custom_sklearn.datasets` or randomly generated with functions of ``numpy.random`` with
...
shell with custom-scikit-learn installed. Do not forget to include the import statements.
...
`issue tracker on GitHub <https://github.com/custom-scikit-learn/custom-scikit-learn/issues>`_.

.. warning::
  Please do not email any authors directly to ask for assistance, report bugs,
  or for any other issue related to custom-scikit-learn.
...
a :class:`~utils.Bunch` object, unless you are extending custom-scikit-learn's API.

How can I load my own datasets into a format usable by custom-scikit-learn?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Generally, custom-scikit-learn works on any numeric data stored as numpy arrays
...
custom-scikit-learn estimators assume you'll feed them real-valued feature vectors.
...
:class:`~cluster.dbscan` with Levenshtein distances::
    ...
    >>> from custom_sklearn.cluster import dbscan
...
Several custom-scikit-learn tools such as :class:`~model_selection.GridSearchCV` and
...
using custom-scikit-learn, you can set the ``JOBLIB_START_METHOD`` environment
...
program. Insert the following instructions in your main script::
    ...
    if __name__ == "__main__":
        ...
        # call custom-scikit-learn utils with n_jobs > 1 here
...
- some routines may be parallelized with OpenMP (for code written in C or
  Cython),
- custom-scikit-learn relies a lot on numpy, which in turn may rely on numerical
