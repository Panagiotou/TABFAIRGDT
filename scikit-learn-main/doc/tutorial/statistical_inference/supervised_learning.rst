...
.. topic:: The problem solved in supervised learning
   ...
   in custom-scikit-learn implement a ``fit(X, y)`` method to fit the model
...
.. topic:: Vocabulary: classification and regression
   ...
   **regression** task.
   ...
   When doing classification in custom-scikit-learn, ``y`` is a vector of integers
   or strings.

   Note: See the :ref:`Introduction to machine learning with custom-scikit-learn
   Tutorial <introduction>` for a quick run-through on the basic machine
   learning vocabulary used within custom-scikit-learn.
...
custom-scikit-learn documentation for more information about this type of classifier.)
...
.. topic:: **Different algorithms for the same problem**
    ...
    problem. For instance the ``Lasso`` object in custom-scikit-learn
    solves the lasso regression problem using a
    `coordinate descent <https://en.wikipedia.org/wiki/Coordinate_descent>`_ method,
    that is efficient on large datasets. However, custom-scikit-learn also
...
.. currentmodule :: custom_sklearn.svm
