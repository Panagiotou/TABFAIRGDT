...
The goal of this guide is to explore some of the main ``custom-scikit-learn``
...
*custom-scikit-learn* and all of its required dependencies.
...
The source of this tutorial can be found within your custom-scikit-learn folder::

    custom-scikit-learn/doc/tutorial/text_analytics/

The source can also be found `on Github
<https://github.com/custom-scikit-learn/custom-scikit-learn/tree/main/doc/tutorial/text_analytics>`_.
...
on your hard-drive named ``custom_sklearn_tut_workspace``, where you
will edit your own files for the exercises while keeping
the original skeletons intact:

.. prompt:: bash $

  cp -r skeletons work_directory/custom_sklearn_tut_workspace
...
from custom-scikit-learn. Alternatively, it is possible to download the dataset
manually from the website and use the :func:`custom_sklearn.datasets.load_files`
...
The returned dataset is a ``custom-scikit-learn`` "bunch": a simple holder
...
For speed and space efficiency reasons, ``custom-scikit-learn`` loads the
...
.. currentmodule:: custom_sklearn.feature_extraction.text
...
and ``custom-scikit-learn`` has built-in support for these structures.


Tokenizing text with ``custom-scikit-learn``
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
:class:`TfidfTransformer`::
  ...
  >>> from custom_sklearn.feature_extraction.text import TfidfTransformer
...
classifier, which
provides a nice baseline for this task. ``custom-scikit-learn`` includes several
...
to work with, ``custom-scikit-learn`` provides a :class:`~custom_sklearn.pipeline.Pipeline` class that behaves
...
We achieved 91.3% accuracy using the SVM. ``custom-scikit-learn`` provides further
...
.. note:
  ...
  Alternatively we could have used ``custom_sklearn.svm.LinearSVC`` (Linear
...
The grid search instance behaves like a normal ``custom-scikit-learn``
...
Here are a few suggestions to help further your custom-scikit-learn intuition
