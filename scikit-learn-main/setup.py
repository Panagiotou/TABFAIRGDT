...
# custom_sklearn __init__ can detect if it is being loaded by the setup routine, to
# avoid attempting to load components that aren't built yet.
# TODO: can this be simplified or removed since the switch to setuptools
# away from numpy.distutils?
builtins.__custom_sklearn_SETUP__ = True


DISTNAME = "custom-scikit-learn"
DESCRIPTION = "A set of python modules for machine learning and data mining"
with open("README.rst") as f:
    LONG_DESCRIPTION = f.read()
MAINTAINER = "custom-scikit-learn developers"
MAINTAINER_EMAIL = "custom-scikit-learn@python.org"
URL = "https://custom-scikit-learn.org"
DOWNLOAD_URL = "https://pypi.org/project/custom-scikit-learn/#files"
LICENSE = "new BSD"
PROJECT_URLS = {
    "Bug Tracker": "https://github.com/custom-scikit-learn/custom-scikit-learn/issues",
    "Documentation": "https://custom-scikit-learn.org/stable/documentation.html",
    "Source Code": "https://github.com/custom-scikit-learn/custom-scikit-learn",
}

# We can actually import a restricted version of custom_sklearn that
# does not need the compiled code
import custom_sklearn  # noqa
import custom_sklearn._min_dependencies as min_deps  # noqa
from custom_sklearn._build_utils import _check_cython_version  # noqa
from custom_sklearn.externals._packaging.version import parse as parse_version  # noqa


VERSION = custom_sklearn.__version__
...
class CleanCommand(Command):
    ...
    def run(self):
        ...
        if os.path.exists("build"):
            shutil.rmtree("build")
        for dirpath, dirnames, filenames in os.walk("custom_sklearn"):
...
class build_ext_subclass(build_ext):
    def finalize_options(self):
        ...
        if self.parallel is None:
            ...
            parallel = os.environ.get("custom_sklearn_BUILD_PARALLEL")
            if parallel:
    ...
    def build_extensions(self):
        from custom_sklearn._build_utils.openmp_helpers import get_openmp_flag
        ...
        if custom_sklearn._OPENMP_SUPPORTED:
...
def check_package_status(package, min_version):
    ...
    req_str = "custom-scikit-learn requires {} >= {}.\n".format(package, min_version)

    instructions = (
        "Installation instructions are available on the "
        "custom-scikit-learn website: "
        "https://custom-scikit-learn.org/stable/install.html\n"
    )
...
libraries = [
    (
        ...
        {
            "sources": [
                join("custom_sklearn", "svm", "src", "libsvm", "libsvm_template.cpp"),
            ],
            "depends": [
                join("custom_sklearn", "svm", "src", "libsvm", "svm.cpp"),
                join("custom_sklearn", "svm", "src", "libsvm", "svm.h"),
                join("custom_sklearn", "svm", "src", "newrand", "newrand.h"),
            ],
    ...
    (
        ...
        {
            "sources": [
                join("custom_sklearn", "svm", "src", "liblinear", "linear.cpp"),
                join("custom_sklearn", "svm", "src", "liblinear", "tron.cpp"),
            ],
            "depends": [
                join("custom_sklearn", "svm", "src", "liblinear", "linear.h"),
                join("custom_sklearn", "svm", "src", "liblinear", "tron.h"),
                join("custom_sklearn", "svm", "src", "newrand", "newrand.h"),
            ],
            # Use C++11 to use the random number generator fix
            "extra_compiler_args": ["-std=c++11"],
...
def configure_extension_modules():
    ...
    from custom_sklearn._build_utils import cythonize_extensions, gen_from_templates
    ...
    build_with_debug_symbols = (
        os.environ.get("custom_sklearn_BUILD_ENABLE_DEBUG_SYMBOLS", "0") != "0"
    )
    if os.name == "posix":
    ...
    for submodule, extensions in extension_config.items():
        ...
        parent_dir = join("custom_sklearn", *submodule_parts)
        for extension in extensions:
            ...
            if submodule:
                name_parts = ["custom_sklearn", submodule, source_name]
            else:
                name_parts = ["custom_sklearn", source_name]
...
def setup_package():
    ...
    if not all(
        ...
        if sys.version_info < required_python_version:
            ...
            raise RuntimeError(
                "custom-scikit-learn requires Python %s or later. The current"
