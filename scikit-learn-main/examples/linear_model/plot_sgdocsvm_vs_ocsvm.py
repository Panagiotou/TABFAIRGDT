...
:class:`custom_sklearn.svm.OneClassSVM` in the case of an RBF kernel with
:class:`custom_sklearn.linear_model.SGDOneClassSVM`, a Stochastic Gradient Descent
(SGD) version of the One-Class SVM. A kernel approximation is first used in
order to apply :class:`custom_sklearn.linear_model.SGDOneClassSVM` which implements a
linear One-Class SVM using SGD.

Note that :class:`custom_sklearn.linear_model.SGDOneClassSVM` scales linearly with
the number of samples whereas the complexity of a kernelized
:class:`custom_sklearn.svm.OneClassSVM` is at best quadratic with respect to the
