...
# should use a :class:`~custom_sklearn.model_selection.TimeSeriesSplit`
...
# -----
#
# In this example, we demo a :class:`~custom_sklearn.linear_model.Lasso` with a fixed
# value of the regularization parameter `alpha`. In practice, the optimal
# parameter `alpha` should be selected by passing a
# :class:`~custom_sklearn.model_selection.TimeSeriesSplit` cross-validation strategy to a
# :class:`~custom_sklearn.linear_model.LassoCV`. To keep the example simple and fast to
# execute, we directly set the optimal value for alpha here.
from time import time

from custom_sklearn.linear_model import Lasso
...
# comparison of :class:`~custom_sklearn.linear_model.ARDRegression` and
# :class:`~custom_sklearn.linear_model.BayesianRidge` regressors.

from custom_sklearn.linear_model import ARDRegression
...
# ----------
#
# :class:`~custom_sklearn.linear_model.ElasticNet` is a middle ground between
# :class:`~custom_sklearn.linear_model.Lasso` and :class:`~custom_sklearn.linear_model.Ridge`,
...
# :class:`~custom_sklearn.linear_model.Ridge`. Similarly, `l1_ratio = 1` is a pure L1
# penalty and the model is equivalent to a :class:`~custom_sklearn.linear_model.Lasso`.
...
# :class:`~custom_sklearn.linear_model.ElasticNetCV`, not shown here to keep the
# example simple.

from custom_sklearn.linear_model import ElasticNet
...
# %%
# In the present example :class:`~custom_sklearn.linear_model.ElasticNet` yields the
...
# :class:`~custom_sklearn.linear_model.ElasticNet` and
# :class:`~custom_sklearn.linear_model.ARDRegression` result in a less sparse model
# than a :class:`~custom_sklearn.linear_model.Lasso`.
#
# Conclusions
# -----------
#
# :class:`~custom_sklearn.linear_model.Lasso` is known to recover sparse data
...
# :class:`~custom_sklearn.linear_model.Lasso` would end up selecting a single one of
# them. In the case of sparse yet non-correlated features, a
# :class:`~custom_sklearn.linear_model.Lasso` model would be more suitable.
#
# :class:`~custom_sklearn.linear_model.ElasticNet` introduces some sparsity on the
...
# less sparse model than a pure :class:`~custom_sklearn.linear_model.Lasso` and may
# capture non-predictive features as well.
#
# :class:`~custom_sklearn.linear_model.ARDRegression` is better when handling gaussian
