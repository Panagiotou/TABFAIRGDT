...
from custom_sklearn.ensemble import GradientBoostingClassifier, RandomForestClassifier
...
# %%
# Notice that :class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` is much
# faster than :class:`~custom_sklearn.ensemble.GradientBoostingClassifier` starting
# with intermediate datasets (`n_samples >= 10_000`), which is not the case of
# the present example.
#
# The :class:`~custom_sklearn.ensemble.RandomTreesEmbedding` is an unsupervised method
...
# regression because it is a standard custom-scikit-learn transformer.
...
# method `apply`. The pipeline in custom-scikit-learn expects a call to `transform`.
# Therefore, we wrapped the call to `apply` within a `FunctionTransformer`.

from custom_sklearn.preprocessing import FunctionTransformer, OneHotEncoder
