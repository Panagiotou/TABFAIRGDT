...
#
# * :class:`~custom_sklearn.linear_model.LogisticRegression` (used as baseline
#   since very often, properly regularized logistic regression is well
#   calibrated by default thanks to the use of the log-loss)
# * Uncalibrated :class:`~custom_sklearn.naive_bayes.GaussianNB`
# * :class:`~custom_sklearn.naive_bayes.GaussianNB` with isotonic and sigmoid
...
from custom_sklearn.calibration import CalibratedClassifierCV, CalibrationDisplay
...
# %%
# Uncalibrated :class:`~custom_sklearn.naive_bayes.GaussianNB` is poorly calibrated
# because of
...
# :class:`~custom_sklearn.naive_bayes.GaussianNB` with :ref:`isotonic` can fix
...
#
# * :class:`~custom_sklearn.linear_model.LogisticRegression` (baseline)
# * Uncalibrated :class:`~custom_sklearn.svm.LinearSVC`. Since SVC does not output
#   probabilities by default, we naively scale the output of the
#   :term:`decision_function` into [0, 1] by applying min-max scaling.
# * :class:`~custom_sklearn.svm.LinearSVC` with isotonic and sigmoid
#   calibration (see :ref:`User Guide <calibration>`)
...
# %%
# :class:`~custom_sklearn.svm.LinearSVC` shows the opposite
# behavior to :class:`~custom_sklearn.naive_bayes.GaussianNB`; the calibration
# curve has a sigmoid shape, which is typical for an under-confident
# classifier. In the case of :class:`~custom_sklearn.svm.LinearSVC`, this is caused
...
# sense that :class:`~custom_sklearn.svm.LinearSVC` does not try to separate samples
...
# %%
# As with :class:`~custom_sklearn.naive_bayes.GaussianNB` above, calibration improves
...
# :class:`~custom_sklearn.svm.LinearSVC`) but not where it is transposed-sigmoid
# (e.g., :class:`~custom_sklearn.naive_bayes.GaussianNB`). Non-parametric
