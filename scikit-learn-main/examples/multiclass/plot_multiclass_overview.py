...
In custom-scikit-learn, all estimators support multiclass classification out of the
box: the most sensible strategy was implemented for the end-user. The
:mod:`custom_sklearn.multiclass` module implements various strategies that one can use
for experimenting or developing third-party estimators that only support binary
classification.

:mod:`custom_sklearn.multiclass` includes OvO/OvR strategies used to train a
multiclass classifier by fitting a set of binary classifiers (the
:class:`~custom_sklearn.multiclass.OneVsOneClassifier` and
:class:`~custom_sklearn.multiclass.OneVsRestClassifier` meta-estimators). This example
will review them.
"""
...
# dataset. We use the :func:`custom_sklearn.datasets.fetch_openml` function to load
...
# :class:`~custom_sklearn.tree.DecisionTreeClassifier` and a
# :class:`~custom_sklearn.model_selection.RepeatedStratifiedKFold` cross-validation
# with 3 splits and 5 repetitions.
#
# We compare the following strategies:
#
# * :class:~custom_sklearn.tree.DecisionTreeClassifier can handle multiclass
...
# * :class:`~custom_sklearn.multiclass.OneVsOneClassifier` trains a set of binary
#   classifiers where each classifier is trained to distinguish between
#   two classes.
# * :class:`~custom_sklearn.multiclass.OneVsRestClassifier`: trains a set of binary
#   classifiers where each classifier is trained to distinguish between
#   one class and the rest of the classes.
# * :class:`~custom_sklearn.multiclass.OutputCodeClassifier`: trains a set of binary
#   classifiers where each classifier is trained to distinguish between
#   a set of classes from the rest of the classes. The set of classes is
#   defined by a codebook, which is randomly generated in custom-scikit-learn. This
...
from custom_sklearn.model_selection import RepeatedStratifiedKFold, cross_validate
from custom_sklearn.multiclass import (
    OneVsOneClassifier,
    OneVsRestClassifier,
...
#
# Finally, it is important to recall that the estimators in custom-scikit-learn
