...
(:class:`~custom_sklearn.decomposition.PCA`) and its kernelized version
(:class:`~custom_sklearn.decomposition.KernelPCA`).

On the one hand, we show that :class:`~custom_sklearn.decomposition.KernelPCA` is able
to find a projection of the data which linearly separates them while it is not the case
with :class:`~custom_sklearn.decomposition.PCA`.

Finally, we show that inverting this projection is an approximation with
:class:`~custom_sklearn.decomposition.KernelPCA`, while it is exact with
...
# kernel.
from custom_sklearn.decomposition import PCA, KernelPCA
...
# :class:`~custom_sklearn.decomposition.KernelPCA` is related to the reconstruction
# (i.e. the back projection in the original feature space). With
# :class:`~custom_sklearn.decomposition.PCA`, the reconstruction will be exact if
...
# :class:`~custom_sklearn.decomposition.PCA` we observe a different result for
# :class:`~custom_sklearn.decomposition.KernelPCA`.
#
# Indeed, :meth:`~custom_sklearn.decomposition.KernelPCA.inverse_transform` cannot
# rely on an analytical back-projection and thus an exact reconstruction.
# Instead, a :class:`~custom_sklearn.kernel_ridge.KernelRidge` is internally trained
...
# :meth:`~custom_sklearn.decomposition.KernelPCA.inverse_transform`, one can tune
# `alpha` in :class:`~custom_sklearn.decomposition.KernelPCA`, the regularization term
