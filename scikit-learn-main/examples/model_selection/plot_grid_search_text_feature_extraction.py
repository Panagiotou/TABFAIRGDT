...
:class:`~custom_sklearn.model_selection.RandomizedSearchCV`. For a demo on the
...
# calling the dataset loader :func:`~custom_sklearn.datasets.fetch_20newsgroups` to get
# the 20 of them.
...
# :class:`~custom_sklearn.model_selection.RandomizedSearchCV`. Using a
# :class:`~custom_sklearn.model_selection.GridSearchCV` instead would explore all the
# possible combinations on the grid, which can be costly to compute, whereas the
# parameter `n_iter` of the :class:`~custom_sklearn.model_selection.RandomizedSearchCV`
