...
We use the :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` to select the
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` in a setting of
...
from custom_sklearn.datasets import fetch_openml

custom_sklearn.set_config(transform_output="pandas")
...
#
# From these four metrics, custom-scikit-learn does not provide a scorer for the FPR. We
...
#
# We therefore need to define a custom-scikit-learn scorer using
# :func:`~custom_sklearn.metrics.make_scorer` where the information is passed. We store all
...
from custom_sklearn.metrics import make_scorer, precision_score, recall_score
...
def credit_gain_score(y, y_pred, neg_label, pos_label):
    ...
    # custom-scikit-learn model selection tools expect that we follow a convention
...
# ^^^^^^^^^^^^^^^^^^^^^^^^
#
# We use :class:`~custom_sklearn.ensemble.HistGradientBoostingClassifier` as a predictive model
...
from custom_sklearn.metrics import PrecisionRecallDisplay, RocCurveDisplay
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` class is here to help us.
...
# ^^^^^^^^^^^^^^^^^^^^^^^^
#
# We use :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` to tune the
...
from custom_sklearn.model_selection import TunedThresholdClassifierCV
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV`. In particular, the
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` is not the same as the
...
# %%
# From this business metric, we create a custom-scikit-learn scorer that given a fitted
# classifier and a test set compute the business metric. In this regard, we use
# the :func:`~custom_sklearn.metrics.make_scorer` factory. The variable `amount` is an
...
from custom_sklearn.metrics import get_scorer
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` to optimize the decision
...
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` is in charge of
...
# beyond the scope of the custom-scikit-learn library itself.
#
# Manually setting the decision threshold instead of tuning it
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#
# In the previous example, we used the
# :class:`~custom_sklearn.model_selection.TunedThresholdClassifierCV` to find the optimal
...
#
# The class :class:`~custom_sklearn.model_selection.FixedThresholdClassifier` allows us to
...
from custom_sklearn.model_selection import FixedThresholdClassifier
