...
# %%
# We define a wrapper class for implementing the custom-scikit-learn API to the
...
class NMSlibTransformer(TransformerMixin, BaseEstimator):
    """Wrapper for using nmslib as custom_sklearn's KNeighborsTransformer"""
...
from custom_sklearn.manifold import TSNE
...
# Both :class:`~custom_sklearn.neighbors.KNeighborsTransformer` and `NMSlibTransformer`
...
#
# We can observe that the default :class:`~custom_sklearn.manifold.TSNE` estimator with
# its internal :class:`~custom_sklearn.neighbors.NearestNeighbors` implementation is
# roughly equivalent to the pipeline with :class:`~custom_sklearn.manifold.TSNE` and
# :class:`~custom_sklearn.neighbors.KNeighborsTransformer` in terms of performance.
# This is expected because both pipelines rely internally on the same
# :class:`~custom_sklearn.neighbors.NearestNeighbors` implementation that performs
...
# implementation significantly improved since custom-scikit-learn 1.1. Furthermore, the
...
# :class:`~custom_sklearn.manifold.TSNE` pipeline, the gains of the approximate search
