...
We will model the complexity influence on three different estimators:
    - :class:`~custom_sklearn.linear_model.SGDClassifier` (for classification data)
      which implements stochastic gradient descent learning;

    - :class:`~custom_sklearn.svm.NuSVR` (for regression data) which implements
      Nu support vector regression;

    - :class:`~custom_sklearn.ensemble.GradientBoostingRegressor` builds an additive
      model in a forward stage-wise fashion. Notice that
      :class:`~custom_sklearn.ensemble.HistGradientBoostingRegressor` is much faster
      than :class:`~custom_sklearn.ensemble.GradientBoostingRegressor` starting with
...
from custom_sklearn.metrics import hamming_loss, mean_squared_error
...
#
# .. note:: We are using
#    :func:`~custom_sklearn.datasets.fetch_20newsgroups_vectorized` to download 20
