...
This is an example showing how the custom-scikit-learn API can be used to cluster
documents by topics using a `Bag of Words approach
<https://en.wikipedia.org/wiki/Bag-of-words_model>`_.

Two algorithms are demonstrated, namely :class:`~custom_sklearn.cluster.KMeans` and its more
scalable variant, :class:`~custom_sklearn.cluster.MiniBatchKMeans`. Additionally,
...
:class:`~custom_sklearn.feature_extraction.text.TfidfVectorizer` and a
:class:`~custom_sklearn.feature_extraction.text.HashingVectorizer`. See the example
...
# the `remove` parameter from :func:`~custom_sklearn.datasets.fetch_20newsgroups` to
...
#
# - :class:`~custom_sklearn.feature_extraction.text.TfidfVectorizer` uses an in-memory
...
#
# - :class:`~custom_sklearn.feature_extraction.text.HashingVectorizer` hashes word
...
# :class:`~custom_sklearn.feature_extraction.text.TfidfVectorizer`.
...
# -----------------------------------
#
# As both :class:`~custom_sklearn.cluster.KMeans` and
# :class:`~custom_sklearn.cluster.MiniBatchKMeans` optimize a non-convex objective
...
from custom_sklearn.cluster import KMeans
...
# :class:`~custom_sklearn.decomposition.TruncatedSVD`, which works on term count/tf-idf
# matrices. Since SVD results are not normalized, we redo the normalization to
# improve the :class:`~custom_sklearn.cluster.KMeans` result. Using SVD to reduce the
...
from custom_sklearn.decomposition import TruncatedSVD
...
# both :class:`~custom_sklearn.cluster.KMeans` and
# :class:`~custom_sklearn.cluster.MiniBatchKMeans`.
...
from custom_sklearn.cluster import MiniBatchKMeans
...
# ---------------------
#
# Since :class:`~custom_sklearn.feature_extraction.text.TfidfVectorizer` can be
...
# :class:`~custom_sklearn.feature_extraction.text.HashingVectorizer` instance, which
...
# :class:`~custom_sklearn.feature_extraction.text.HashingVectorizer` output to a
# :class:`~custom_sklearn.feature_extraction.text.TfidfTransformer` instance. In this
# case we also add LSA to the pipeline to reduce the dimension and sparcity of
# the hashed vector space.

from custom_sklearn.feature_extraction.text import HashingVectorizer, TfidfTransformer
...
# %%
# :class:`~custom_sklearn.cluster.KMeans` and :class:`~custom_sklearn.cluster.MiniBatchKMeans`
...
#
# The size of the error bars show that :class:`~custom_sklearn.cluster.MiniBatchKMeans`
# is less stable than :class:`~custom_sklearn.cluster.KMeans` for this relatively small
