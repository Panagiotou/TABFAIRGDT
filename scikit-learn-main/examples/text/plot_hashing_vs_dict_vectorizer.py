...
We first compare :func:`~custom_sklearn.feature_extraction.FeatureHasher` and
:func:`~custom_sklearn.feature_extraction.DictVectorizer` by using both methods to
...
:func:`~custom_sklearn.feature_extraction.text.HashingVectorizer`,
:func:`~custom_sklearn.feature_extraction.text.CountVectorizer` and
:func:`~custom_sklearn.feature_extraction.text.TfidfVectorizer` that handle both the
...
# --------------
#
# First we benchmark the :func:`~custom_sklearn.feature_extraction.DictVectorizer`,
# then we compare it to :func:`~custom_sklearn.feature_extraction.FeatureHasher` as
# both of them receive dictionaries as input.

from time import time

from custom_sklearn.feature_extraction import DictVectorizer
...
# :func:`~custom_sklearn.feature_extraction.FeatureHasher` is 2**20. Here we set
# `n_features = 2**18` to illustrate hash collisions.
#
# **FeatureHasher on frequency dictionaries**

from custom_sklearn.feature_extraction import FeatureHasher
...
# :func:`~custom_sklearn.feature_extraction.FeatureHasher` is lower than those obtained
# using the :func:`~custom_sklearn.feature_extraction.DictVectorizer`. This is due to
# hash collisions.
#
...
# unique terms found by the :func:`~custom_sklearn.feature_extraction.DictVectorizer`.
#
# **FeatureHasher on raw tokens**
#
# Alternatively, one can set `input_type="string"` in the
# :func:`~custom_sklearn.feature_extraction.FeatureHasher` to vectorize the strings
...
# %%
# In both cases :func:`~custom_sklearn.feature_extraction.FeatureHasher` is
# approximately twice as fast as
# :func:`~custom_sklearn.feature_extraction.DictVectorizer`. This is handy when dealing
...
# ------------------------------------------------
#
# :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` accepts raw data as
# it internally implements tokenization and occurrence counting. It is similar
# to the :func:`~custom_sklearn.feature_extraction.DictVectorizer` when used along with
# the customized function `token_freqs` as done in the previous section. The
# difference being that :func:`~custom_sklearn.feature_extraction.text.CountVectorizer`
...
# %%
# We see that using the :func:`~custom_sklearn.feature_extraction.text.CountVectorizer`
# implementation is approximately twice as fast as using the
# :func:`~custom_sklearn.feature_extraction.DictVectorizer` along with the simple
# function we defined for mapping the tokens. The reason is that
# :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` is optimized by
...
# :func:`~custom_sklearn.feature_extraction.text.HashingVectorizer`, which is
# equivalent to combining the "hashing trick" implemented by the
# :func:`~custom_sklearn.feature_extraction.FeatureHasher` class and the text
# preprocessing and tokenization of the
# :func:`~custom_sklearn.feature_extraction.text.CountVectorizer`.

from custom_sklearn.feature_extraction.text import HashingVectorizer
...
# :func:`~custom_sklearn.feature_extraction.text.TfidfTransformer`. TF stands for
# "term-frequency" while "tf-idf" means term-frequency times inverse
# document-frequency.
#
# We now benchmark the :func:`~custom_sklearn.feature_extraction.text.TfidfVectorizer`,
# which is equivalent to combining the tokenization and occurrence counting of
# the :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` along with the
# normalizing and weighting from a
# :func:`~custom_sklearn.feature_extraction.text.TfidfTransformer`.
...
# :func:`~custom_sklearn.feature_extraction.text.TfidfVectorizer` is slightly slower
# than :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` because of the
# extra operation induced by the
# :func:`~custom_sklearn.feature_extraction.text.TfidfTransformer`.
#
# Also notice that, by setting the number of features `n_features = 2**18`, the
# :func:`~custom_sklearn.feature_extraction.text.HashingVectorizer` performs better
# than the :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` at the
# expense of inversibility of the transformation due to hash collisions.
#
# We highlight that :func:`~custom_sklearn.feature_extraction.text.CountVectorizer` and
# :func:`~custom_sklearn.feature_extraction.text.HashingVectorizer` perform better than
# their equivalent :func:`~custom_sklearn.feature_extraction.DictVectorizer` and
# :func:`~custom_sklearn.feature_extraction.FeatureHasher` on manually tokenized
