...
# our dataset. We fit a :class:`~custom_sklearn.linear_model.Ridge` and check the
...
# We can use a :class:`~custom_sklearn.gaussian_process.kernels.ExpSineSquared` kernel
# which allows recovering the periodicity. The class
# :class:`~custom_sklearn.kernel_ridge.KernelRidge` will accept such a kernel.
#
...
# "kernel trick".
#
# Thus, let's use such a :class:`~custom_sklearn.kernel_ridge.KernelRidge`.
import time
...
#
# Now, we will use a
# :class:`~custom_sklearn.gaussian_process.GaussianProcessRegressor` to fit the same
...
# :class:`~custom_sklearn.gaussian_process.kernels.WhiteKernel` that is used to
