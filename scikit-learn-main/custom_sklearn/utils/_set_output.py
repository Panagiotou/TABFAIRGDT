...
def _get_adapter_from_container(container):
    ...
    See :class:`custom_sklearn.utils._set_output.ContainerAdapterProtocol` for more
    details.
    """
    ...
    try:
        ...
        raise ValueError(
            "The container does not have a registered adapter in custom-scikit-learn. "
...
def _get_output_config(method, estimator=None):
    ...
    config : dict
        ...
        - "dense": specifies the dense container for `method`. This can be
          `"default"` or `"pandas"`.
    """
    est_custom_sklearn_output_config = getattr(estimator, "_custom_sklearn_output_config", {})
    if method in est_custom_sklearn_output_config:
        dense_config = est_custom_sklearn_output_config[method]
    else:
...
def _auto_wrap_is_configured(estimator):
    ...
    `_SetOutputMixin` sets `_custom_sklearn_auto_wrap_output_keys` to `set()` if auto wrapping
    is manually disabled.
    """
    auto_wrap_output_keys = getattr(estimator, "_custom_sklearn_auto_wrap_output_keys", set())
    return (
...
class _SetOutputMixin:
    ...
    def __init_subclass__(cls, auto_wrap_output_keys=("transform",), **kwargs):
        ...
        if auto_wrap_output_keys is None:
            cls._custom_sklearn_auto_wrap_output_keys = set()
            return
        ...
        for method, key in method_to_key.items():
            if not hasattr(cls, method) or key not in auto_wrap_output_keys:
                continue
            cls._custom_sklearn_auto_wrap_output_keys.add(key)
    ...
    def set_output(self, *, transform=None):
        ...
        if not hasattr(self, "_custom_sklearn_output_config"):
            self._custom_sklearn_output_config = {}

        self._custom_sklearn_output_config["transform"] = transform
        return self
