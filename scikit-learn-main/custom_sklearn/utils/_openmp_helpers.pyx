...
def _openmp_parallelism_enabled():
    """Determines whether custom-scikit-learn has been built with OpenMP

    It allows to retrieve at runtime the information gathered at compile time.
    """
    # custom_sklearn_OPENMP_PARALLELISM_ENABLED is resolved at compile time and defined
    # in _openmp_helpers.pxd as a boolean. This function exposes it to Python.
    return custom_sklearn_OPENMP_PARALLELISM_ENABLED
...
cpdef _openmp_effective_n_threads(n_threads=None, only_physical_cores=True):
    ...
    If custom-scikit-learn is built without OpenMP support, always return 1.
    """
    if n_threads == 0:
        raise ValueError("n_threads = 0 is invalid")

    if not custom_sklearn_OPENMP_PARALLELISM_ENABLED:
