...
from custom_sklearn.utils._array_api import _check_array_api_dispatch
...
from custom_sklearn.utils.multiclass import check_classification_targets
...
# assert_raises_regex but lets keep the backward compat in custom-scikit-learn with
...
def assert_allclose(
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.utils._testing import assert_allclose
    >>> x = [1e-5, 1e-3, 1e-1]
...
def check_skip_network():
    if int(os.environ.get("custom_sklearn_SKIP_NETWORK_TESTS", 0)):
...
def create_memmap_backed_data(data, mmap_mode="r", return_folder=False):
    ...
    """
    temp_folder = tempfile.mkdtemp(prefix="custom_sklearn_testing_")
...
def check_docstring_parameters(func, doc=None, ignore=None):
    ...
    if not func_name.startswith("custom_sklearn.") or func_name.startswith(
        "custom_sklearn.externals"
    ):
        return incorrect
...
def assert_run_python_script_without_output(source_code, pattern=".+", timeout=60):
    ...
    """
    fd, source_file = tempfile.mkstemp(suffix="_src_test_custom_sklearn.py")
    os.close(fd)
    try:
        with open(source_file, "wb") as f:
            f.write(source_code.encode("utf-8"))
        cmd = [sys.executable, source_file]
        cwd = op.normpath(op.join(op.dirname(custom_sklearn.__file__), ".."))
        env = os.environ.copy()
        try:
