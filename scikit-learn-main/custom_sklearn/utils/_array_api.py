...
def supported_float_dtypes(xp):
    ...
    time of writing but custom-scikit-learn estimators and functions can choose
...
class _ArrayAPIWrapper:
    """custom_sklearn specific Array API compatibility wrapper

    This wrapper makes it possible for custom-scikit-learn maintainers to
...
class _NumPyAPIWrapper:
    ...
    API while working with any version of NumPy supported by custom-scikit-learn.

    See the `get_namespace()` public function for more details.
    """

    # TODO: once custom-scikit-learn drops support for NumPy < 2, this class can be
    # removed, assuming Array API compliance of NumPy 2 is actually sufficient
    # for custom-scikit-learn's needs.
...
def get_namespace(*arrays, remove_none=True, remove_types=(str,), xp=None):
    ...
    Namespace support is not enabled by default. To enabled it call:
      ...
      custom_sklearn.set_config(array_api_dispatch=True)

    or:

      with custom_sklearn.config_context(array_api_dispatch=True):
          # your code here
    ...
    *arrays : array objects
    ...
    # array-api-compat is a required dependency of custom-scikit-learn only when
...
def _average(a, axis=None, weights=None, normalize=True, xp=None):
    ...
    only for the common cases needed in custom-scikit-learn.
    """
...
def _asarray_with_order(
    ...
    however some input validation code in custom-scikit-learn needs to work both
