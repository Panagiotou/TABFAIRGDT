...
from custom_sklearn import config_context
from custom_sklearn.base import BaseEstimator
from custom_sklearn.cluster import AgglomerativeClustering, Birch
from custom_sklearn.compose import ColumnTransformer, make_column_transformer
from custom_sklearn.datasets import load_iris
from custom_sklearn.decomposition import PCA, TruncatedSVD
from custom_sklearn.ensemble import StackingClassifier, StackingRegressor, VotingClassifier
from custom_sklearn.feature_selection import SelectPercentile
from custom_sklearn.gaussian_process.kernels import ExpSineSquared
from custom_sklearn.impute import SimpleImputer
from custom_sklearn.kernel_ridge import KernelRidge
...
from custom_sklearn.neural_network import MLPClassifier
from custom_sklearn.pipeline import FeatureUnion, Pipeline, make_pipeline
from custom_sklearn.preprocessing import OneHotEncoder, StandardScaler
from custom_sklearn.svm import LinearSVC, LinearSVR
from custom_sklearn.tree import DecisionTreeClassifier
from custom_sklearn.utils._estimator_html_repr import (
    _get_css_style,
    _get_visual_block,
...
from custom_sklearn.utils.fixes import parse_version
...
def test_html_documentation_link_mixin_custom_sklearn(mock_version):
    """Check the behaviour of the `_HTMLDocumentationLinkMixin` class for custom-scikit-learn
    default.
    """

    # mock the `__version__` where the mixin is located
    with patch("custom_sklearn.utils._estimator_html_repr.__version__", mock_version):
        mixin = _HTMLDocumentationLinkMixin()

        assert mixin._doc_link_module == "custom_sklearn"
        custom_sklearn_version = parse_version(mock_version)
        # we need to parse the version manually to be sure that this test is passing in
        # other branches than `main` (that is "dev").
        if custom_sklearn_version.dev is None:
            version = f"{custom_sklearn_version.major}.{custom_sklearn_version.minor}"
        else:
            version = "dev"
        assert (
            mixin._doc_link_template
            == f"https://custom-scikit-learn.org/{version}/modules/generated/"
            "{estimator_module}.{estimator_name}.html"
        )
        assert (
            mixin._get_doc_link()
            == f"https://custom-scikit-learn.org/{version}/modules/generated/"
            "custom_sklearn.utils._HTMLDocumentationLinkMixin.html"
        )
...
def test_non_utf8_locale(set_non_utf8_locale):
    ...
    Non-regression test for https://github.com/custom-scikit-learn/custom-scikit-learn/issues/27725
    """
    _get_css_style()
