...
from custom_sklearn.utils.parallel import Parallel, delayed
...
def test_parallel_delayed_warnings():
    """Informative warnings should be raised when mixing custom_sklearn and joblib API"""
    # We should issue a warning when one wants to use custom_sklearn.utils.fixes.Parallel
    # with joblib.delayed. The config will not be propagated to the workers.
    warn_msg = "`custom_sklearn.utils.parallel.Parallel` needs to be used in conjunction"
    with pytest.warns(UserWarning, match=warn_msg) as records:
        Parallel()(joblib.delayed(time.sleep)(0) for _ in range(10))
    assert len(records) == 10

    # We should issue a warning if one wants to use custom_sklearn.utils.fixes.delayed with
    # joblib.Parallel
    warn_msg = (
        "`custom_sklearn.utils.parallel.delayed` should be used with "
        "`custom_sklearn.utils.parallel.Parallel` to make it possible to propagate"
    )
    with pytest.warns(UserWarning, match=warn_msg) as records:
...
def test_dispatch_config_parallel(n_jobs):
    ...
    https://github.com/custom-scikit-learn/custom-scikit-learn/issues/25239
    """
