...
def estimator_html_repr(estimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.utils._estimator_html_repr import estimator_html_repr
...
class _HTMLDocumentationLinkMixin:
    ...
    - `_doc_link_module`: it corresponds to the root module (e.g. `custom_sklearn`). Using this
      mixin, the default value is `custom_sklearn`.
    - `_doc_link_template`: it corresponds to the template used to generate the
      ...
      `"https://custom-scikit-learn.org/{version_url}/modules/generated/
    ...
    :func:`custom_sklearn.utils.estimator_html_repr` to generate a link to the API
    ...
    _doc_link_module = "custom_sklearn"
    _doc_link_url_param_generator = None

    @property
    def _doc_link_template(self):
        custom_sklearn_version = parse_version(__version__)
        if custom_sklearn_version.dev is None:
            version_url = f"{custom_sklearn_version.major}.{custom_sklearn_version.minor}"
        else:
            version_url = "dev"
        return getattr(
            self,
            "__doc_link_template",
            (
                f"https://custom-scikit-learn.org/{version_url}/modules/generated/"
                "{estimator_module}.{estimator_name}.html"
            ),
        )
    ...
    def _get_doc_link(self):
        ...
        if self._doc_link_url_param_generator is None:
            ...
            # This works because in custom-scikit-learn all public estimators are exposed at
