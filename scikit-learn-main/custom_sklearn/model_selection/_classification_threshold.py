...
class FixedThresholdClassifier(BaseThresholdClassifier):
    ...
    See Also
    --------
    custom_sklearn.model_selection.TunedThresholdClassifierCV : Classifier that post-tunes
        the decision threshold based on some metrics and using cross-validation.
    custom_sklearn.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
    ...
    >>> from custom_sklearn.model_selection import FixedThresholdClassifier, train_test_split
...
class TunedThresholdClassifierCV(BaseThresholdClassifier):
    ...
    scoring : str or callable, default="balanced_accuracy"
        ...
        * a scorer callable object created with :func:`~custom_sklearn.metrics.make_scorer`;
    ...
    See Also
    --------
    custom_sklearn.model_selection.FixedThresholdClassifier : Classifier that uses a
        constant threshold.
    custom_sklearn.calibration.CalibratedClassifierCV : Estimator that calibrates
        probabilities.
    ...
    >>> from custom_sklearn.model_selection import TunedThresholdClassifierCV, train_test_split
    >>> X, y = make_classification(
    ...     n_samples=1_000, weights=[0.9, 0.1], class_sep=0.8, random_state=42
    ... )
    >>> X_train, X_test, y_train, y_test = train_test_split(
