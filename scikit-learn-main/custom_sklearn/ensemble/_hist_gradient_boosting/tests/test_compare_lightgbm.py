...
from custom_sklearn.datasets import make_classification, make_regression
...
from custom_sklearn.ensemble._hist_gradient_boosting.utils import get_equivalent_estimator
...
def test_same_predictions_regression(
    ...
    # Make sure custom_sklearn has the same predictions as lightgbm for easy targets.
    #
    # In particular when the size of the trees are bound and the number of
    # samples is large enough, the structure of the prediction trees found by
    # LightGBM and custom_sklearn should be exactly identical.
    #
    # Notes:
    ...
    est_custom_sklearn = HistGradientBoostingRegressor(
        loss=loss,
        max_iter=max_iter,
        max_bins=max_bins,
        learning_rate=1,
    ...
    est_lightgbm = get_equivalent_estimator(est_custom_sklearn, lib="lightgbm")
    est_lightgbm.set_params(min_sum_hessian_in_leaf=0)

    est_lightgbm.fit(X_train, y_train)
    est_custom_sklearn.fit(X_train, y_train)

    # We need X to be treated an numerical data, not pre-binned data.
    X_train, X_test = X_train.astype(np.float32), X_test.astype(np.float32)

    pred_lightgbm = est_lightgbm.predict(X_train)
    pred_custom_sklearn = est_custom_sklearn.predict(X_train)
    if loss in ("gamma", "poisson"):
        ...
        assert (
            np.mean(np.isclose(pred_lightgbm, pred_custom_sklearn, rtol=1e-2, atol=1e-2))
            > 0.65
        )
    else:
        # Less than 1% of the predictions may deviate more than 1e-3 in relative terms.
        assert np.mean(np.isclose(pred_lightgbm, pred_custom_sklearn, rtol=1e-3)) > 1 - 0.01

    if max_leaf_nodes < 10 and n_samples >= 1000 and loss in ("squared_error",):
        pred_lightgbm = est_lightgbm.predict(X_test)
        pred_custom_sklearn = est_custom_sklearn.predict(X_test)
        # Less than 1% of the predictions may deviate more than 1e-4 in relative terms.
        assert np.mean(np.isclose(pred_lightgbm, pred_custom_sklearn, rtol=1e-4)) > 1 - 0.01
...
def test_same_predictions_classification(
    ...
    est_custom_sklearn = HistGradientBoostingClassifier(
        loss="log_loss",
        max_iter=max_iter,
        max_bins=max_bins,
        learning_rate=1,
    ...
    est_lightgbm = get_equivalent_estimator(
        est_custom_sklearn, lib="lightgbm", n_classes=n_classes
    )
    ...
    pred_lightgbm = est_lightgbm.predict(X_train)
    pred_custom_sklearn = est_custom_sklearn.predict(X_train)
    assert np.mean(pred_custom_sklearn == pred_lightgbm) > 0.89

    acc_lightgbm = accuracy_score(y_train, pred_lightgbm)
    acc_custom_sklearn = accuracy_score(y_train, pred_custom_sklearn)
    np.testing.assert_almost_equal(acc_lightgbm, acc_custom_sklearn)
    ...
    if max_leaf_nodes < 10 and n_samples >= 1000:
        ...
        acc_custom_sklearn = accuracy_score(y_test, pred_custom_sklearn)
        np.testing.assert_almost_equal(acc_lightgbm, acc_custom_sklearn, decimal=2)
...
def test_same_predictions_multiclass_classification(
    ...
    proba_custom_sklearn = est_custom_sklearn.predict_proba(X_train)
    # assert more than 75% of the predicted probabilities are the same up to
    # the second decimal
    assert np.mean(np.abs(proba_lightgbm - proba_custom_sklearn) < 1e-2) > 0.75

    acc_lightgbm = accuracy_score(y_train, pred_lightgbm)
    acc_custom_sklearn = accuracy_score(y_train, pred_custom_sklearn)

    np.testing.assert_allclose(acc_lightgbm, acc_custom_sklearn, rtol=0, atol=5e-2)
