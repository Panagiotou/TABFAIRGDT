...
from custom_sklearn.metrics import DistanceMetric
from custom_sklearn.metrics._dist_metrics import (
    BOOL_METRICS,
    DistanceMetric32,
    DistanceMetric64,
)
from custom_sklearn.utils import check_random_state
from custom_sklearn.utils._testing import assert_allclose, create_memmap_backed_data
from custom_sklearn.utils.fixes import CSR_CONTAINERS, parse_version, sp_version
...
def test_cdist(metric_param_grid, X, Y, csr_container):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        if metric == "mahalanobis" and X.dtype == np.float32:
            ...
            # the scipy and custom-scikit-learn implementation.
        ...
        D_custom_sklearn = dm.pairwise(X, Y)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn, D_scipy_cdist, **rtol_dict)

        D_custom_sklearn = dm.pairwise(X_csr, Y_csr)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn, D_scipy_cdist, **rtol_dict)

        D_custom_sklearn = dm.pairwise(X_csr, Y)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn, D_scipy_cdist, **rtol_dict)

        D_custom_sklearn = dm.pairwise(X, Y_csr)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn, D_scipy_cdist, **rtol_dict)
...
def test_cdist_bool_metric(metric, X_bool, Y_bool, csr_container):
    ...
    D_custom_sklearn = dm.pairwise(X_bool, Y_bool)
    assert_allclose(D_custom_sklearn, D_scipy_cdist)
    ...
    D_custom_sklearn = dm.pairwise(X_bool_csr, Y_bool_csr)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_scipy_cdist)

    D_custom_sklearn = dm.pairwise(X_bool, Y_bool_csr)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_scipy_cdist)

    D_custom_sklearn = dm.pairwise(X_bool_csr, Y_bool)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_scipy_cdist)
...
def test_pdist(metric_param_grid, X, csr_container):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        dm = DistanceMetric.get_metric(metric, X.dtype, **kwargs)
        D_custom_sklearn = dm.pairwise(X)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn, D_scipy_pdist, **rtol_dict)

        D_custom_sklearn_csr = dm.pairwise(X_csr)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn_csr, D_scipy_pdist, **rtol_dict)

        D_custom_sklearn_csr = dm.pairwise(X_csr, X_csr)
        assert D_custom_sklearn.flags.c_contiguous
        assert_allclose(D_custom_sklearn_csr, D_scipy_pdist, **rtol_dict)
...
def test_pdist_bool_metrics(metric, X_bool, csr_container):
    ...
    dm = DistanceMetric.get_metric(metric)
    D_custom_sklearn = dm.pairwise(X_bool)
    assert_allclose(D_custom_sklearn, D_scipy_pdist)

    X_bool_csr = csr_container(X_bool)
    D_custom_sklearn = dm.pairwise(X_bool_csr)
    assert_allclose(D_custom_sklearn, D_scipy_pdist)
...
def test_haversine_metric(X, Y, csr_container):
    ...
    D_custom_sklearn = haversine.pairwise(X, Y)
    assert_allclose(
        haversine.dist_to_rdist(D_custom_sklearn), np.sin(0.5 * D_reference) ** 2, rtol=1e-6
    )

    assert_allclose(D_custom_sklearn, D_reference)

    D_custom_sklearn = haversine.pairwise(X_csr, Y_csr)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_reference)

    D_custom_sklearn = haversine.pairwise(X_csr, Y)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_reference)

    D_custom_sklearn = haversine.pairwise(X, Y_csr)
    assert D_custom_sklearn.flags.c_contiguous
    assert_allclose(D_custom_sklearn, D_reference)
...
def test_readonly_kwargs():
    ...
    # https://github.com/custom-scikit-learn/custom-scikit-learn/issues/21685
