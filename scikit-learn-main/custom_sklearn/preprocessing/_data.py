...
def _is_constant_feature(var, mean, n_samples):
    ...
    """
    # In custom-scikit-learn, variance is always computed using float64 accumulators.
...
def scale(X, *, axis=0, with_mean=True, with_std=True, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~custom_sklearn.preprocessing.scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.StandardScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(StandardScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from custom_sklearn.preprocessing import scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class MinMaxScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import MinMaxScaler
...
def minmax_scale(X, feature_range=(0, 1), *, axis=0, copy=True):
    ...
    .. versionadded:: 0.17
       *minmax_scale* function interface
       to :class:`~custom_sklearn.preprocessing.MinMaxScaler`.
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~custom_sklearn.preprocessing.minmax_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.MinMaxScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import minmax_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class StandardScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    :class:`~custom_sklearn.decomposition.PCA` : Further removes the linear
    ...
    def partial_fit(self, X, y=None, sample_weight=None):
        ...
        if sparse.issparse(X):
            ...
            if self.with_std:
                ...
                # See https://github.com/custom-scikit-learn/custom-scikit-learn/pull/12338
...
class MaxAbsScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import MaxAbsScaler
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
def maxabs_scale(X, *, axis=0, copy=True):
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~custom_sklearn.preprocessing.maxabs_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.MaxAbsScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import maxabs_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class RobustScaler(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    custom_sklearn.decomposition.PCA : Further removes the linear correlation across
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import RobustScaler
    >>> X = [[ 1., -2.,  2.],
    ...      [ -2.,  1.,  3.],
    ...      [ 4.,  1., -2.]]
...
def robust_scale(
    ...
    *,
    ...
    RobustScaler : Performs centering and scaling using the Transformer API
        (e.g. as part of a preprocessing :class:`~custom_sklearn.pipeline.Pipeline`).
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~custom_sklearn.preprocessing.robust_scale` unless you know
        what you are doing. A common mistake is to apply it to the entire data
        *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.RobustScaler` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
        leaking: `pipe = make_pipeline(RobustScaler(), LogisticRegression())`.

    Examples
    --------
    >>> from custom_sklearn.preprocessing import robust_scale
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
def normalize(X, norm="l2", *, axis=1, copy=True, return_norm=False):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import normalize
    >>> X = [[-2, 1, 2], [-1, 0, 1]]
...
class Normalizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import Normalizer
    >>> X = [[4, 1, 2, 2],
    ...      [1, 3, 9, 3],
    ...      [5, 7, 5, 1]]
    ...
    def fit(self, X, y=None):
        ...
        (ii) be consistent with the custom-scikit-learn transformer API.
...
def binarize(X, *, threshold=0.0, copy=True):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import binarize
...
class Binarizer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import Binarizer
    >>> X = [[ 1., -1.,  2.],
    ...      [ 2.,  0.,  0.],
    ...      [ 0.,  1., -1.]]
...
class KernelCenterer(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    for :class:`~custom_sklearn.decomposition.KernelPCA` for instance.
    ...
    See Also
    --------
    custom_sklearn.kernel_approximation.Nystroem : Approximate a kernel map
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import KernelCenterer
...
def add_dummy_feature(X, value=1.0):
    ...
    Examples
    --------
    >>> from custom_sklearn.preprocessing import add_dummy_feature
...
class QuantileTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.preprocessing import QuantileTransformer
...
def quantile_transform(
    ...
    *,
    ...
    .. warning:: Risk of data leak
        ...
        Do not use :func:`~custom_sklearn.preprocessing.quantile_transform` unless
        ...
        training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.QuantileTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.preprocessing import quantile_transform
    >>> rng = np.random.RandomState(0)
    >>> X = np.sort(rng.normal(loc=0.5, scale=0.25, size=(25, 1)), axis=0)
...
class PowerTransformer(OneToOneFeatureMixin, TransformerMixin, BaseEstimator):
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.preprocessing import PowerTransformer
    >>> pt = PowerTransformer()
    >>> data = [[1, 2], [3, 2], [4, 5]]
    >>> print(pt.fit(data))
    PowerTransformer()
...
def power_transform(X, method="yeo-johnson", *, standardize=True, copy=True):
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.preprocessing import power_transform
    >>> data = [[1, 2], [3, 2], [4, 5]]
    ...
    .. warning:: Risk of data leak.
        Do not use :func:`~custom_sklearn.preprocessing.power_transform` unless you
        know what you are doing. A common mistake is to apply it to the entire
        data *before* splitting into training and test sets. This will bias the
        model evaluation because information would have leaked from the test
        set to the training set.
        In general, we recommend using
        :class:`~custom_sklearn.preprocessing.PowerTransformer` within a
        :ref:`Pipeline <pipeline>` in order to prevent most risks of data
