...
from custom_sklearn import config_context, set_config
from custom_sklearn._min_dependencies import PYTEST_MIN_VERSION
...
from custom_sklearn.tests import random_seed
from custom_sklearn.utils._testing import get_pytest_filterwarning_lines
...
def raccoon_face_or_skip():
    ...
    if scipy_datasets_require_network:
        run_network_tests = environ.get("custom_sklearn_SKIP_NETWORK_TESTS", "1") == "0"
        if not run_network_tests:
            raise SkipTest("test is enabled when custom_sklearn_SKIP_NETWORK_TESTS=0")
...
_SKIP32_MARK = pytest.mark.skipif(
    environ.get("custom_sklearn_RUN_FLOAT32_TESTS", "0") != "1",
    reason="Set custom_sklearn_RUN_FLOAT32_TESTS=1 to run float32 dtype tests",
)
...
def _fetch_fixture(f):
    ...
    download_if_missing = environ.get("custom_sklearn_SKIP_NETWORK_TESTS", "1") == "0"
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            if str(e) != "Data not found and `download_if_missing` is False":
                raise
            pytest.skip("test is enabled when custom_sklearn_SKIP_NETWORK_TESTS=0")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_network = pytest.mark.skip(
        reason="test is enabled when custom_sklearn_SKIP_NETWORK_TESTS=0"
    )
    ...
    for item in items:
        ...
        if (
            ...
            marker = pytest.mark.xfail(
                reason=(
                    "know failure. See "
                    "https://github.com/custom-scikit-learn/custom-scikit-learn/issues/17797"  # noqa
                )
            )
    ...
    if skip_doctests:
        ...
        for item in items:
            if isinstance(item, DoctestItem):
                ...
                # details.
                if item.name != "custom_sklearn._config.config_context":
    ...
    if not pillow_installed:
        ...
        for item in items:
            if item.name in [
                "custom_sklearn.feature_extraction.image.PatchExtractor",
...
def pytest_configure(config):
    ...
    if not config.pluginmanager.hasplugin("custom_sklearn.tests.random_seed"):
        config.pluginmanager.register(random_seed)

    if environ.get("custom_sklearn_WARNINGS_AS_ERRORS", "0") != "0":
