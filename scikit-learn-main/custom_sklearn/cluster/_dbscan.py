...
def dbscan(
    ...
    *,
    ...
    <custom_sklearn.neighbors.NearestNeighbors.radius_neighbors_graph>` with
    ``mode='distance'``, then using ``metric='precomputed'`` here.

    Another way to reduce memory and computation time is to remove
    (near-)duplicate points and use ``sample_weight`` instead.

    :class:`~custom_sklearn.cluster.OPTICS` provides a similar clustering with lower
    memory usage.
...
class DBSCAN(ClusterMixin, BaseEstimator):
    ...
    :class:`~custom_sklearn.cluster.OPTICS` provides a similar clustering with lower memory
    usage.
