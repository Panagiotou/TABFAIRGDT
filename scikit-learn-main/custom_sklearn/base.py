...
def clone(estimator, *, safe=True):
    ...
    .. versionchanged:: 1.3
        Delegates to `estimator.__custom_sklearn_clone__` if the method exists.
    ...
    safe : bool, default=True
        ...
        that are not estimators. Ignored if `estimator.__custom_sklearn_clone__`
        exists.
    ...
    Examples
    --------
    >>> from custom_sklearn.base import clone
    ...
    False
    """
    if hasattr(estimator, "__custom_sklearn_clone__") and not inspect.isclass(estimator):
        return estimator.__custom_sklearn_clone__()
    return _clone_parametrized(estimator, safe=safe)


def _clone_parametrized(estimator, *, safe=True):
    """Default implementation of clone. See :func:`custom_sklearn.base.clone` for details."""
        ...
        if not safe:
            ...
            if isinstance(estimator, type):
                raise TypeError(
                    ...
                    + "custom-scikit-learn estimator instead of a class."
                )
            else:
                raise TypeError(
                    "Cannot clone object '%s' (type %s): "
                    "it does not seem to be a custom-scikit-learn "
    ...
    # _custom_sklearn_output_config is used by `set_output` to configure the output
    # container of an estimator.
    if hasattr(estimator, "_custom_sklearn_output_config"):
        new_object._custom_sklearn_output_config = copy.deepcopy(
            estimator._custom_sklearn_output_config
        )
    return new_object


class BaseEstimator(_HTMLDocumentationLinkMixin, _MetadataRequester):
    """Base class for all estimators in custom-scikit-learn.
    ...
    def _get_param_names(cls):
        ...
        for p in parameters:
            if p.kind == p.VAR_POSITIONAL:
                raise RuntimeError(
                    "custom-scikit-learn estimators should always "
    ...
    def set_params(self, **params):
        ...
        (such as :class:`~custom_sklearn.pipeline.Pipeline`). The latter have
    ...
    def __getstate__(self):
        ...
        if type(self).__module__.startswith("custom_sklearn."):
            return dict(state.items(), _custom_sklearn_version=__version__)
        else:
            return state

    def __setstate__(self, state):
        if type(self).__module__.startswith("custom_sklearn."):
            pickle_version = state.pop("_custom_sklearn_version", "pre-0.18")
            if pickle_version != __version__:
                warnings.warn(
                    InconsistentVersionWarning(
                        estimator_name=self.__class__.__name__,
                        current_custom_sklearn_version=__version__,
                        original_custom_sklearn_version=pickle_version,
                    ),
                )
        try:
    ...
    def _validate_data(
        ...
        **check_params : kwargs
            Parameters passed to :func:`custom_sklearn.utils.check_array` or
            :func:`custom_sklearn.utils.check_X_y`. Ignored if validate_separately
            is not False.
...
class ClassifierMixin:
    """Mixin class for all classifiers in custom-scikit-learn.

    This mixin defines the following functionality:

    - `_estimator_type` class attribute defaulting to `"classifier"`;
    - `score` method that default to :func:`~custom_sklearn.metrics.accuracy_score`.
...
class RegressorMixin:
    """Mixin class for all regression estimators in custom-scikit-learn.

    This mixin defines the following functionality:

    - `_estimator_type` class attribute defaulting to `"regressor"`;
    - `score` method that default to :func:`~custom_sklearn.metrics.r2_score`.
    - enforce that `fit` requires `y` to be passed through the `requires_y` tag.
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import BaseEstimator, RegressorMixin
    >>> # Mixin classes should always be on the left-hand side for a correct MRO
    ...
    def score(self, X, y, sample_weight=None):
        ...
        with default value of :func:`~custom_sklearn.metrics.r2_score`.
        This influences the ``score`` method of all the multioutput
        regressors (except for
        :class:`~custom_sklearn.multioutput.MultiOutputRegressor`).
        """
...
class ClusterMixin:
    """Mixin class for all cluster estimators in custom-scikit-learn.
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import BaseEstimator, ClusterMixin
...
class BiclusterMixin:
    """Mixin class for all bicluster estimators in custom-scikit-learn.
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import BaseEstimator, BiclusterMixin
...
class TransformerMixin(_SetOutputMixin):
    """Mixin class for all transformers in custom-scikit-learn.
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import BaseEstimator, TransformerMixin
...
class OneToOneFeatureMixin:
    ...
    and output features, such as :class:`~custom_sklearn.preprocessing.StandardScaler`.

    Examples
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import OneToOneFeatureMixin
...
class ClassNamePrefixFeaturesOutMixin:
    ...
    names out, such as :class:`~custom_sklearn.decomposition.PCA`. For example, if
    :class:`~custom_sklearn.decomposition.PCA` outputs 3 features, then the generated feature
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import ClassNamePrefixFeaturesOutMixin
...
class DensityMixin:
    """Mixin class for all density estimators in custom-scikit-learn.
    ...
    Examples
    --------
    >>> from custom_sklearn.base import DensityMixin
...
class OutlierMixin:
    """Mixin class for all outlier detection estimators in custom-scikit-learn.
    ...
    --------
    >>> import numpy as np
    >>> from custom_sklearn.base import BaseEstimator, OutlierMixin
...
class MetaEstimatorMixin:
    """Mixin class for all meta estimators in custom-scikit-learn.
    ...
    Examples
    --------
    >>> from custom_sklearn.base import MetaEstimatorMixin
...
def is_classifier(estimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.base import is_classifier
    >>> from custom_sklearn.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
...
def is_regressor(estimator):
    ...
    Examples
    --------
    >>> from custom_sklearn.base import is_regressor
    >>> from custom_sklearn.svm import SVC, SVR
    >>> classifier = SVC()
    >>> regressor = SVR()
