...
See the documentation for the custom_sklearn_TESTS_GLOBAL_RANDOM_SEED
variable for insrtuctions on how to use this fixture.

https://custom-scikit-learn.org/dev/computing/parallelism.html#custom_sklearn-tests-global-random-seed
"""
...
def pytest_configure(config):
    ...
    random_seed_var = environ.get("custom_sklearn_TESTS_GLOBAL_RANDOM_SEED")
    if hasattr(config, "workerinput") and "random_seeds" in config.workerinput:
        ...
        if min(random_seeds) < 0 or max(random_seeds) > 99:
            raise ValueError(
                ...
                "custom_sklearn_TESTS_GLOBAL_RANDOM_SEED must be in the range [0, 99] "
...
def pytest_report_header(config):
    ...
    if random_seed_var == "any":
        return [
            ...
            f'    custom_sklearn_TESTS_GLOBAL_RANDOM_SEED="{config.option.random_seeds[0]}"',
            (
                "See: https://custom-scikit-learn.org/dev/computing/parallelism.html"
                "#custom_sklearn-tests-global-random-seed"
            ),
        ]
