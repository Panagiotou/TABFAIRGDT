"""
General tests for all estimators in custom_sklearn.
"""
...
from custom_sklearn.cluster import (
    OPTICS,
    AffinityPropagation,
    Birch,
    MeanShift,
...
from custom_sklearn.exceptions import ConvergenceWarning, FitFailedWarning

# make it possible to discover experimental estimators when calling `all_estimators`
from custom_sklearn.experimental import (
    enable_halving_search_cv,  # noqa
    enable_iterative_imputer,  # noqa
)
from custom_sklearn.linear_model import LogisticRegression, Ridge
from custom_sklearn.linear_model._base import LinearClassifierMixin
from custom_sklearn.manifold import TSNE, Isomap, LocallyLinearEmbedding
from custom_sklearn.model_selection import (
    GridSearchCV,
    HalvingGridSearchCV,
    HalvingRandomSearchCV,
    RandomizedSearchCV,
)
from custom_sklearn.neighbors import (
...
from custom_sklearn.preprocessing import (
    FunctionTransformer,
    MinMaxScaler,
    OneHotEncoder,
    StandardScaler,
)
from custom_sklearn.semi_supervised import LabelPropagation, LabelSpreading
from custom_sklearn.utils import all_estimators
from custom_sklearn.utils._tags import _DEFAULT_TAGS, _safe_tags
from custom_sklearn.utils._testing import (
    SkipTest,
    ignore_warnings,
    set_random_state,
)
from custom_sklearn.utils.estimator_checks import (
    _construct_instance,
...
from custom_sklearn.utils.fixes import _IS_PYPY, _IS_WASM
...
def test_setup_py_check():
    ...
    # custom-scikit-learn source tree.
    cwd = os.getcwd()
    setup_path = Path(custom_sklearn.__file__).parent.parent
...
def test_import_all_consistency():
    custom_sklearn_path = [os.path.dirname(custom_sklearn.__file__)]
    # Smoke test to check that any name in a __all__ list is actually defined
    # in the namespace of the module or package.
    pkgs = pkgutil.walk_packages(
        path=custom_sklearn_path, prefix="custom_sklearn.", onerror=lambda _: None
    )
    submods = [modname for _, modname, _ in pkgs]
    for modname in submods + ["custom_sklearn"]:
        if ".tests." in modname:
            continue
        # Avoid test suite depending on setuptools
        if "custom_sklearn._build_utils" in modname:
            continue
        if _IS_PYPY and (
...
def test_root_import_all_completeness():
    ...
    for _, modname, _ in pkgutil.walk_packages(
        path=custom_sklearn_path, onerror=lambda _: None
    ):
        if "." in modname or modname.startswith("_") or modname in EXCEPTIONS:
            continue
        assert modname in custom_sklearn.__all__


@pytest.mark.skipif(
    custom_sklearn._BUILT_WITH_MESON,
    reason=(
...
def test_all_tests_are_importable():
    ...
    resource_modules = {
        "custom_sklearn.datasets.data",
        "custom_sklearn.datasets.descr",
        "custom_sklearn.datasets.images",
    }
    custom_sklearn_path = [os.path.dirname(custom_sklearn.__file__)]
    lookup = {
        name: ispkg
        for _, name, ispkg in pkgutil.walk_packages(custom_sklearn_path, prefix="custom_sklearn.")
    }
    missing_tests = [
        name
...
def test_f_contiguous_array_estimator(Estimator):
    ...
    # https://github.com/custom-scikit-learn/custom-scikit-learn/issues/23988
    # https://github.com/custom-scikit-learn/custom-scikit-learn/issues/24013
