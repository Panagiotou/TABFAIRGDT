...
from custom_sklearn.utils.deprecation import _is_deprecated
from custom_sklearn.utils.estimator_checks import (
    _construct_instance,
    _enforce_estimator_tags_X,
    _enforce_estimator_tags_y,
)
from custom_sklearn.utils.fixes import _IS_PYPY, parse_version, sp_version
...
with warnings.catch_warnings():
    ...
    PUBLIC_MODULES = set(
        [
            pckg[1]
            for pckg in walk_packages(prefix="custom_sklearn.", path=custom_sklearn_path)
            if not ("._" in pckg[1] or ".tests." in pckg[1])
        ]
    )

# functions to ignore args / docstring of
# TODO(1.7): remove "custom_sklearn.utils._joblib"
_DOCSTRING_IGNORES = [
    "custom_sklearn.utils.deprecation.load_mlcomp",
    "custom_sklearn.pipeline.make_pipeline",
    "custom_sklearn.pipeline.make_union",
    "custom_sklearn.utils.extmath.safe_sparse_dot",
    "custom_sklearn.utils._joblib",
    "HalfBinomialLoss",
]
...
def test_docstring_parameters():
    ...
    for name in PUBLIC_MODULES:
        if name.endswith(".conftest"):
            # pytest tooling, not part of the custom-scikit-learn API
            continue
        if name == "custom_sklearn.utils.fixes":
            # We cannot always control these docstrings
            continue
        with warnings.catch_warnings(record=True):
            module = importlib.import_module(name)
        classes = inspect.getmembers(module, inspect.isclass)
        # Exclude non-custom-scikit-learn classes
        classes = [cls for cls in classes if cls[1].__module__.startswith("custom_sklearn")]
...
@ignore_warnings(category=custom_sklearn.exceptions.ConvergenceWarning)
