...
from custom_sklearn import config_context, datasets
from custom_sklearn.base import (
    BaseEstimator,
    OutlierMixin,
    TransformerMixin,
    clone,
...
from custom_sklearn.utils._mocking import MockDataFrame
from custom_sklearn.utils._set_output import _get_output_config
...
class VargEstimator(BaseEstimator):
    """custom-scikit-learn estimators shouldn't have vargs."""
...
def test_clone():
    ...
    from custom_sklearn.feature_selection import SelectFpr, f_classif
...
def test_clone_class_rather_than_instance():
    ...
    msg = "You should provide an instance of custom-scikit-learn estimator"
...
def test_clone_protocol():
    """Checks that clone works with `__custom_sklearn_clone__` protocol."""
...
def test_pickle_version_warning_is_not_raised_with_matching_version():
    ...
    assert b"_custom_sklearn_version" in tree_pickle
...
class TreeBadVersion(DecisionTreeClassifier):
    def __getstate__(self):
        return dict(self.__dict__.items(), _custom_sklearn_version="something")
...
def test_pickle_version_warning_is_issued_upon_different_version():
    ...
    message = pickle_error_message.format(
        ...
        current_version=custom_sklearn.__version__,
    )
    ...
    assert message.original_custom_sklearn_version == "something"
    assert message.current_custom_sklearn_version == custom_sklearn.__version__
...
def test_pickle_version_warning_is_issued_when_no_version_info_in_pickle():
    ...
    assert b"_custom_sklearn_version" not in tree_pickle_noversion
    message = pickle_error_message.format(
...
def test_pickle_version_no_warning_is_issued_with_non_custom_sklearn_estimator():
    iris = datasets.load_iris()
    tree = TreeNoVersion().fit(iris.data, iris.target)
    tree_pickle_noversion = pickle.dumps(tree)
    try:
        module_backup = TreeNoVersion.__module__
        TreeNoVersion.__module__ = "notcustom_sklearn"
...
def test_pickling_when_getstate_is_overwritten_by_mixin_outside_of_custom_sklearn():
    try:
        estimator = MultiInheritanceEstimator()
        text = "this attribute should not be pickled"
        estimator._attribute_not_pickled = text
        old_mod = type(estimator).__module__
        type(estimator).__module__ = "notcustom_sklearn"
...
def test_estimator_empty_instance_dict(estimator):
    ...
    expected = {"_custom_sklearn_version": custom_sklearn.__version__}
