...
from custom_sklearn.datasets import fetch_openml as fetch_openml_orig
from custom_sklearn.datasets._openml import (
    _OPENML_PREFIX,
    _get_local_path,
    _open_openml_url,
    _retry_with_clean_cache,
)
from custom_sklearn.utils import Bunch
from custom_sklearn.utils._optional_dependencies import check_pandas_support
...
OPENML_TEST_DATA_MODULE = "custom_sklearn.datasets.tests.data.openml"
...
# the mock data in custom_sklearn/datasets/tests/data/openml/ is not always consistent
...
def _monkey_patch_webbased_functions(context, data_id, gzip_response):
    ...
    # XXX: Global variable
    if test_offline:
        context.setattr(custom_sklearn.datasets._openml, "urlopen", _mock_urlopen)
...
# https://github.com/custom-scikit-learn/custom-scikit-learn/issues/18906
@fails_if_pypy
...
def test_open_openml_url_cache(monkeypatch, gzip_response, tmpdir):
    ...
    _monkey_patch_webbased_functions(monkeypatch, data_id, gzip_response)
    openml_path = custom_sklearn.datasets._openml._DATA_FILE.format(data_id)
...
def test_open_openml_url_unlinks_local_path(monkeypatch, tmpdir, write_to_disk):
    ...
    monkeypatch.setattr(custom_sklearn.datasets._openml, "urlopen", _mock_urlopen)
...
def test_fetch_openml_cache(monkeypatch, gzip_response, tmpdir):
    ...
    monkeypatch.setattr(custom_sklearn.datasets._openml, "urlopen", _mock_urlopen_raise)
...
def test_fetch_openml_verify_checksum(monkeypatch, as_frame, cache, tmpdir, parser):
    ...
    mocked_openml_url = custom_sklearn.datasets._openml.urlopen
    ...
    monkeypatch.setattr(custom_sklearn.datasets._openml, "urlopen", swap_file_mock)
...
def test_open_openml_url_retry_on_network_error(monkeypatch):
    ...
    monkeypatch.setattr(
        custom_sklearn.datasets._openml, "urlopen", _mock_urlopen_network_error
    )
...
def test_fetch_openml_with_ignored_feature(monkeypatch, gzip_response, parser):
    ...
    https://github.com/custom-scikit-learn/custom-scikit-learn/issues/14340
    """
    if parser == "pandas":
        pytest.importorskip("pandas")
    data_id = 62
    ...
    dataset = custom_sklearn.datasets.fetch_openml(
...
def test_fetch_openml_strip_quotes(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/custom-scikit-learn/custom-scikit-learn/issues/23381
    """
    pd = pytest.importorskip("pandas")
    data_id = 40966
    _monkey_patch_webbased_functions(monkeypatch, data_id=data_id, gzip_response=False)
...
def test_fetch_openml_leading_whitespace(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/custom-scikit-learn/custom-scikit-learn/issues/25311
    """
    pd = pytest.importorskip("pandas")
    data_id = 1590
    _monkey_patch_webbased_functions(monkeypatch, data_id=data_id, gzip_response=False)
...
def test_fetch_openml_quotechar_escapechar(monkeypatch):
    ...
    Non-regression test for:
    https://github.com/custom-scikit-learn/custom-scikit-learn/issues/25478
    """
    pd = pytest.importorskip("pandas")
    data_id = 42074
    _monkey_patch_webbased_functions(monkeypatch, data_id=data_id, gzip_response=False)
