...
from custom_sklearn.datasets._base import (
    RemoteFileMetadata,
    _fetch_remote,
    load_csv_data,
    load_gzip_compressed_csv_data,
)
from custom_sklearn.datasets.tests.test_common import check_as_frame
...
def test_bunch_pickle_generated_with_0_16_and_read_with_0_17():
    ...
    # with custom-scikit-learn 0.16 and are read with 0.17. Basically there
    # is a surprising behaviour because reading bunch.key uses
    # bunch.__dict__ (which is non empty for 0.16 Bunch objects)
    # whereas assigning into bunch.key uses bunch.__setattr__. See
    # https://github.com/custom-scikit-learn/custom-scikit-learn/issues/6196 for
    # more details
...
def test_load_boston_error():
    ...
    with pytest.raises(ImportError, match=msg):
        from custom_sklearn.datasets import load_boston  # noqa

    # other non-existing function should raise the usual import error
    msg = "cannot import name 'non_existing_function' from 'custom_sklearn.datasets'"
    with pytest.raises(ImportError, match=msg):
        from custom_sklearn.datasets import non_existing_function  # noqa


def test_fetch_remote_raise_warnings_with_invalid_url(monkeypatch):
    """Check retry mechanism in _fetch_remote."""

    url = "https://custom-scikit-learn.org/this_file_does_not_exist.tar.gz"
    ...
    monkeypatch.setattr("custom_sklearn.datasets._base.urlretrieve", urlretrieve_mock)
