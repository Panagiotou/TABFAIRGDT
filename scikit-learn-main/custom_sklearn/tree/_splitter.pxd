...
cdef class Splitter:
    ...
    cdef BOOL_t[::1] cat_pos              # which of these features is categorical?
    cdef INT32_t[::1] cat_maxval           # max category
    cdef DTYPE_t[::1] feature_values     # temp. array holding feature values

    cdef SIZE_t[::1] samples_backup             # Sample indices in X, y 2
    cdef DTYPE_t[::1] feature_values_backup    # temp. array holding feature values 2

    cdef SIZE_t start                    # Start position for the current node
    cdef SIZE_t end                      # End position for the current node

    cdef const DOUBLE_t[:, ::1] y
    cdef const DOUBLE_t[:, ::1] s
    cdef double alpha
    ...
    # Methods
    cdef int init(self, object X, const DOUBLE_t[:, ::1] y, const DOUBLE_t[:, ::1] s, double alpha, 
