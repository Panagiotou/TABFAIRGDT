...
cdef class TreeBuilder:
    ...
    cdef inline _check_input(self, object X, cnp.ndarray y, cnp.ndarray s,
...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(self, Tree tree, object X, cnp.ndarray y, cnp.ndarray s, double alpha,
        ...
        # check input
        X, y, s, sample_weight = self._check_input(X, y, s, sample_weight)
        ...
        splitter.init(X, y, s, alpha, sample_weight_ptr)
        ...
        with nogil:
            ...
            while not builder_stack.empty():
                ...
                node_id = tree._add_node(parent, is_left, is_leaf, split.feature,
                                         split.threshold, split.partition,
...
cdef inline BOOL_t _compare_records(
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    cpdef build(self, Tree tree, object X, cnp.ndarray y, cnp.ndarray s, double alpha,
        ...
        cdef DOUBLE_t global_impurity = -1
        ...
        with nogil:
            ...
            #printf("[_tree.pyx] add root to frontier\n")
            rc = self._add_split_node(splitter, tree, 0, n_node_samples,
                                      INFINITY, IS_FIRST, IS_LEFT, NULL, 0,
                                      &split_node_left)
            if rc >= 0:
                _add_to_frontier(split_node_left, frontier)
                global_impurity = tree.nodes[0].impurity

                printf("\033[0;31m\n")
                printf("$ Initial global impurity: %.5f\n", global_impurity)
                printf("\033[0m\n")
            ...
            while not frontier.empty():
                ...
                frontier.pop_back()
                # printf("[_tree.pyx] pop heap\n")

                node = &tree.nodes[record.node_id]
                is_leaf = (record.is_leaf or max_split_nodes <= 0)

                if is_leaf:
                    #printf("Not splitting node %d... Leaf case\n", record.node_id)
                    #printf("[_tree.pyx] is leaf\n")
                    # Node is not expandable; set node as leaf
                    node.left_child = _TREE_LEAF
                    node.right_child = _TREE_LEAF

                    # (these can be arbitrary for leaf so it doesn't matter)
                    node.feature = _TREE_UNDEFINED
                    node.threshold = _TREE_UNDEFINED

                else:
                    printf("\n===============================================\n")
                    printf("Splitting node %d!", record.node_id)
                    global_impurity -= record.improvement
                    printf("\033[0;31m\n$ Global impurity: %.5f\n\033[0m\n", global_impurity)
                    printf("Computing improvements in case L/R kid is further split.\n")
                    #printf("[_tree.pyx] is expandable\n")
                    # Node is expandable

                    # Decrement number of split nodes available
                    max_split_nodes -= 1

                    # Add left child and compute its potential for improvement
                    ...
                    # Add right child and compute its potential for improvement
    ...
    cdef inline int _add_split_node(self, Splitter splitter, Tree tree,
        ...
        bitset_all0(split.partition.side)
        bitset_all0(split.partition.present)
        ...
        if not is_leaf:
            ...
            #printf("split: feature=%d, pos=%d, improvement=%f\n", split.feature, split.pos, split.improvement)
        ...
        node_id = tree._add_node(parent - tree.nodes
                                 ...
                                 is_left, is_leaf,
                                 split.feature, split.threshold, split.partition,
...
cdef class Tree:
    ...
    property n_sens:
        def __get__(self):
            return sizet_ptr_to_ndarray(self.n_sens, self.n_outputs)
    ...
    property partition:
        def __get__(self):
            return self._get_node_ndarray()['partition'][:self.node_count]
    ...
    def __cinit__(self, int n_features, cnp.ndarray n_classes, cnp.ndarray n_sens, int n_outputs, cnp.ndarray cat_pos, cnp.ndarray cat_maxval):
        """Constructor."""
        cdef SIZE_t dummy = 0
        ...
        safe_realloc(&self.n_sens, n_outputs)
    ...
    def __reduce__(self):
        ...
        return (Tree, (self.n_features,
                       ...
                       sizet_ptr_to_ndarray(self.n_sens, self.n_outputs),
    ...
    cdef SIZE_t _add_node(self, SIZE_t parent, bint is_left, bint is_leaf,
                          ...
                          CategoricalPartition partition,
        ...
        if is_leaf:
            ...
            for i in range(8):
                node.partition.side[i] = partition.side[i]
                node.partition.present[i] = partition.present[i]
    ...
    cdef inline cnp.ndarray _apply_dense(self, object X):
        ...
        cdef SIZE_t n_unpresent_right = 0
        cdef SIZE_t n_unpresent_left = 0

        for i in range(n_samples):
            node = self.nodes
            # While node not a leaf
            while node.left_child != _TREE_LEAF:
                is_cat = self.cat_pos[node.feature]
                # ... and node.right_child != _TREE_LEAF:
                
                if is_cat and bitset_get(node.partition.present, <INT32_t>round(X_ndarray[i, node.feature])) == 0:
                    if goes_left(X_ndarray[i, node.feature], is_cat, node.threshold, &node.partition):
                        n_unpresent_left += 1
                    else:
                        n_unpresent_right += 1
                    #printf("/warning: unpresent cat/")
        ...
        #printf("*Unpresency stats: %d/%d samples (%d going left)\n", n_unpresent_left+n_unpresent_right, n_samples, n_unpresent_left)
    ...
    cdef inline object _decision_path_dense(self, object X):
        ...
        for i in range(n_samples):
            ...
            while node.left_child != _TREE_LEAF:
                ...
                if goes_left(X_ndarray[i, node.feature], self.cat_pos[node.feature], node.threshold, &node.partition):
                    node = &self.nodes[node.left_child]
                else:
                    node = &self.nodes[node.right_child]
    ...
    cdef inline object _decision_path_sparse_csr(self, object X):
        ...
        for i in range(n_samples):
            ...
            while node.left_child != _TREE_LEAF:
                ...
                if goes_left(feature_value, self.cat_pos[node.feature], node.threshold, &node.partition):
                    node = &self.nodes[node.left_child]
                else:
                    node = &self.nodes[node.right_child]
            ...
            while stack_size > 0:
                ...
                if current_node.left_child == _TREE_LEAF:
                    ...
                    if is_target_feature:
                        ...
                        if goes_left(X[sample_idx, feature_idx], self.cat_pos[current_node.feature], current_node.threshold, &current_node.partition):
    ...
    with nogil:
        ...
        while not prune_stack.empty():
            ...
            new_node_id = tree._add_node(
                parent, is_left, is_leaf, node.feature, node.threshold, node.partition,
