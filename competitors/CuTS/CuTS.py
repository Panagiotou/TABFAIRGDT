...
import copy
import pandas as pd
from .customizable_synthesizer import CuTS_class
...
class CuTS():
    ...
    def generate(self, size_of_fake_data):
        ...
        dataset = self.synthesizer.dataset
          
        synthetic_dataset = self.synthesizer.generate_data(size_of_fake_data)
        ...
        Xtrain_synth_dec = self.synthesizer.dataset.decode_full_one_hot_batch(Xtrain_synth)

        ytrain_synth_dec = ytrain_synth.cpu().numpy().reshape(-1, 1)

        syndata_dec = np.concatenate((Xtrain_synth_dec, ytrain_synth_dec), axis=-1)

        syndata_df = pd.DataFrame(syndata_dec, columns=self.cols)
        return syndata_df
    


    def fit(self, df, lamda=0.5, workload="all_three_with_labels", dataset_name=""):
        ...
        program_name = f'fairness_downstream_{dataset_name}_sex.txt'

        program_setups = {'arguments': {'param1': 0.0009}, 'denoiser_config': {'finetuning_epochs': 200}}


        load_path = f'competitors/CuTS/{program_name}'
        ...
        denoiser_config = program_setups['denoiser_config']


        self.synthesizer = CuTS_class(
            df = df,
            constraint_program=program, 
            target= self.target_class,
            workload=translated_workload[workload], 
            random_seed=self.seed, 
            device=self.device,
            denoiser_config=denoiser_config
        )

        current_arguments = program_setups['arguments']

        self.cols = list(df.columns)

        self.synthesizer.fit(program_arguments=current_arguments, verbose=True)
