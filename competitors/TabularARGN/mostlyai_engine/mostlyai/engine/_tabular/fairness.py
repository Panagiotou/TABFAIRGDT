...
def apply_fairness_transforms(
    ...
    fairness_transforms: FairnessTransforms,
    lamda: float = 1.0  # Add lambda parameter with default of 0.0 (no fairness)
) -> torch.Tensor:
    """
    Apply fairness transforms to the generated probabilities of the target column.
    
    Args:
        sub_col: The current sub column.
        probs: The probabilities for the current sub column.
        outputs: The sampled values for the sensitive columns.
        fairness_transforms: The transform callables to apply.
        lamda: Parameter between 0 and 1 controlling fairness strength.
                     0 = original distribution
                     1 = fully fair distribution
    """
    device = probs.device
    if sub_col == fairness_transforms["target_sub_col"]:
        # Store original probabilities before transformation
        original_probs = probs.clone()
        
        # Apply transforms as before
        ...
        for i, i_transforms in fairness_transforms["transforms"].items():
            for group, transform in i_transforms.items():
                ...
                # Apply transform only to relevant rows
                transformed_probs = transform(probs[:, i])
                # Use lambda to mix original and transformed probabilities
                probs[:, i] = torch.where(
                    mask,
                    lamda * transformed_probs + (1 - lamda) * original_probs[:, i],
                    probs[:, i]
                )
